...
**Status: RESOLVED 2026-05-07 (PR #134).** Fix candidate 1 (Quote + escape) applied as a conditional: paths containing whitespace, `"`, or `\` are wrapped in double quotes with C-style escapes; bare paths stay unquoted to match the on-disk shape stock distro installs have today. Additionally, `%` is doubled to `%%` and `$` is doubled to `$$`, both regardless of quoting, because systemd expands both `%X` specifiers (`%T` → `/tmp`, `%h` → home, etc.) and `$VAR` / `${VAR}` environment references in `ExecStart` at unit-load time — without the `%%` / `$$` escapes, an install path like `/home/me/100% Tools/dbxignored` or `/home/me/$TOOLS/dbxignored` is silently rewritten by systemd's expander and the unit points at the wrong binary. New helper `_quote_exec_start_path(exe_path)` is the seam. The pre-existing `_escape_systemd_env_value` was renamed to `_escape_systemd_quoted_string` since the same C-style escape rules apply to both `Environment="KEY=VALUE"` and `ExecStart="/path with space"`; the helper is now reused from both call sites. Eight regression tests in `tests/test_linux_systemd.py` cover the whitespace, whitespace-with-arguments, escape-needing-chars, percent-with-whitespace, percent-bare, dollar-with-whitespace, dollar-bare, and unquoted-bare-path branches. Both the percent-escape and dollar-escape branches were added in response to Codex bot reviews on PR #134. Validate-and-refuse (candidate 2) was rejected because quoting is the systemd-idiomatic answer and works in all cases the user might reasonably hit.
...
- **#44 + #82** in PR #134 — install-layer correctness fixes for unusual install paths, bundled. **#44:** `build_task_xml` now passes `getpass.getuser()`, `str(exe_path)`, and `arguments` through `xml.sax.saxutils.escape` so paths/usernames containing `&`, `<`, or `>` produce well-formed XML that `schtasks /Create /XML` accepts. **#82:** `build_unit_content` quotes the `ExecStart` path in double quotes (with C-style `\"` / `\\` escapes for embedded quotes/backslashes) when the path contains whitespace or other escape-needing chars; bare paths stay unquoted to match the on-disk shape stock distro installs have today. Literal `%` and `$` in the path are doubled to `%%` and `$$` regardless of quoting so neither systemd's `%X` specifier expander nor its `$VAR` / `${VAR}` environment expander silently rewrites the install path (both added in response to Codex bot reviews on PR #134). Helper `_quote_exec_start_path` is the seam; `_escape_systemd_env_value` was renamed to `_escape_systemd_quoted_string` since the same C-style escape rules apply to both `Environment="KEY=VALUE"` and `ExecStart="/path with space"`. Eleven new tests across `tests/test_install.py` (XML escape) and `tests/test_linux_systemd.py` (quoting + percent-escape + dollar-escape branches); ten failed against the un-fixed code.
