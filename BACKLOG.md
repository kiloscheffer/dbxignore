...
**Status: RESOLVED 2026-05-08 (PR #144).** Took fix candidate (1) — dropped the guard. The 5-line `if not python: raise RuntimeError(...)` block is gone; the function now ends with `python = shutil.which("python3") or sys.executable; return Path(python), "-m dbxignore daemon"`. Docstring's "Raises RuntimeError…" line removed; the new docstring describes the actual behavior (which became platform-conditional in the same PR per item #50). No test changes needed — none of the existing tests exercised the dead branch.
