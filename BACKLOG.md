...
**Urgency:** medium-low. Fundamental to evaluating an N-dir tree once; reproduces only on fresh installs of personal-account-sized trees. Hourly steady-state sweeps are unaffected. Bundle the "ready-before-sweep" candidate (lowest-risk) with the next daemon-touching change.
...
- **#53** — Initial-sweep wall-clock on a fresh install (no existing markers) was 49.62s on a 27k-dir tree, blocking systemd readiness for ~50s. Reframed 2026-05-08 in PR #157 after the original "marker-present pruning" fix was shown to already exist (pinned by `test_does_not_descend_into_marked_subtree`). Steady-state hourly sweeps are already O(unmarked dirs); the remaining pain is the one-time first sweep. Three new candidates filed: ready-before-sweep (lowest-risk, ~30 LOC), persisted sweep-complete hint (~80 LOC), per-subdir worker fan-out (~40 LOC).
