...
**Candidate 1 (ready-before-sweep) RESOLVED 2026-05-08 (PR #162).** Candidates 2 (persisted sweep-complete hint) and 3 (per-subdir worker fan-out) remain open; the body below is preserved as future-reference for whichever lands next. Wall-clock for the initial sweep itself is unchanged after #162 — what changed is *when* readiness signals fire relative to the sweep, which addressed the user-visible systemd-startup pause. See the Resolved-section entry for the full set of behavior changes that shipped.
...
- **#53** — Initial-sweep wall-clock on a fresh install (no existing markers) was 49.62s on a 27k-dir tree, blocking systemd readiness for ~50s. Candidate 1 (ready-before-sweep) shipped in PR #162 and removed the readiness-pause symptom; the sweep wall-clock itself is unchanged. Two candidates remain open: persisted sweep-complete hint (~80 LOC; reliability concerns on network FS / File Provider mtime semantics), per-subdir worker fan-out (~40 LOC; needs thread-safety review of `RuleCache._applicable` under finer-grained concurrency). Either is a real wall-clock reduction; neither has a fired trigger yet.
