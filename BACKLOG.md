...
The same predicate weakness produces the opposite symptom on `install/macos_launchd.py:uninstall_agent`'s post-bootout wait (added in PR #298): when state.json's PID is stale AND the OS has recycled it to an unrelated python-named process (pytest, an IDE worker, a jupyter kernel), `is_daemon_alive(stale_pid, create_time=None)` returns True on every poll and burns the full 30s timeout + WARNING — even though the daemon really is gone. False-negative on Windows, false-positive on macOS, both rooted in the name-only fallback's permissiveness when there's no create_time to disambiguate.
...
4. **Tighten `state.is_daemon_alive`'s name-only fallback.** When `create_time` is None, the current path treats any pid whose `proc.name()` contains `python` (or matches `_DAEMON_PROCESS_NAMES`) as the daemon. Add a follow-up identity check: `proc.cmdline()` must contain `"dbxignore"`. Resolves both the Windows false-negative above (a stale pid resolving to a non-dbxignore python no longer matters because the daemon-alive check still returns False — but for the right reason) and the macOS false-positive above (a pid recycled to an unrelated python is correctly rejected). Tradeoff: `cmdline` is process-supplied (not OS-authoritative), so a same-named-but-hostile process could spoof — acceptable for a daemon-uninstall liveness probe where the worst case is the existing race. Independent of fix 1; can ship without waiting for the root-cause trace.
...
Apple Developer Program enrollment ($99/yr) for the Developer ID
signature + notarization that make it pass Gatekeeper cleanly. If an
account is obtained, a notarized `.pkg` becomes viable — and the same
account would let the pre-built binary be notarized too, removing the
Gatekeeper friction entirely.

**Urgency:** low. Awaits an Apple Developer account.
...
- **#16** — Windows daemon occasionally writes `daemon_create_time: null` to `state.json`; non-deterministic, observed only on Windows. Silently disables PID-reuse-race protection in `is_daemon_alive`, defeats `uninstall_task`'s wait loop when state.json's PID is stale (Windows false-negative), AND spins `macos_launchd.uninstall_agent`'s post-bootout wait for the full 30s when a stale PID resolves to an unrelated python process (macOS false-positive). `daemon._capture_create_time` wraps the capture with a narrow catch + WARNING; a null observation in the wild carries forensic data in `daemon.log`.
