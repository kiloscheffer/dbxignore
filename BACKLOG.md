...
**Status: RESOLVED 2026-05-07 (PR #134).** Fix candidate 1 (Quote + escape) applied as a conditional: paths containing whitespace, `"`, or `\` are wrapped in double quotes with C-style escapes; bare paths stay unquoted to match the on-disk shape stock distro installs have today. Additionally, `%` is doubled to `%%` regardless of quoting because systemd expands `%X` specifiers in `ExecStart` at unit-load time (`%T` → `/tmp`, `%h` → home, etc.) — without the `%%` escape, an install path like `/home/me/100% Tools/dbxignored` is silently rewritten by the specifier expander and the unit points at the wrong binary. New helper `_quote_exec_start_path(exe_path)` is the seam. The pre-existing `_escape_systemd_env_value` was renamed to `_escape_systemd_quoted_string` since the same C-style escape rules apply to both `Environment="KEY=VALUE"` and `ExecStart="/path with space"`; the helper is now reused from both call sites. Six regression tests in `tests/test_linux_systemd.py` cover the whitespace, whitespace-with-arguments, escape-needing-chars, percent-with-whitespace, percent-bare, and unquoted-bare-path branches. The percent-escape branch was added in response to a Codex bot review on PR #134. Validate-and-refuse (candidate 2) was rejected because quoting is the systemd-idiomatic answer and works in all cases the user might reasonably hit.
