...
Nine items. Six are passive (no concrete trigger requires action); item #32 has one fired trigger (a beta tester typed `dbxignore --version` and got "no such option") but isn't blocking; item #33 is HIGH severity (affects most macOS users with modern Dropbox installs); item #34 is a recurrence of an already-resolved flake (item #18) and waits for a third recurrence before triggering action.
