...
**Status: RESOLVED 2026-05-07 (PR #134).** Fix candidate 1 (Quote + escape) applied as a conditional: paths containing whitespace, `"`, or `\` are wrapped in double quotes with C-style escapes; bare paths stay unquoted to match the on-disk shape stock distro installs have today. New helper `_quote_exec_start_path(exe_path)` is the seam. The pre-existing `_escape_systemd_env_value` was renamed to `_escape_systemd_quoted_string` since the same C-style escape rules apply to both `Environment="KEY=VALUE"` and `ExecStart="/path with space"`; the helper is now reused from both call sites. Four regression tests in `tests/test_linux_systemd.py` cover the whitespace, whitespace-with-arguments, escape-needing-chars, and unquoted-bare-path branches. Validate-and-refuse (candidate 2) was rejected because quoting is the systemd-idiomatic answer and works in all cases the user might reasonably hit.
