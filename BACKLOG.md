...
**Status: RESOLVED 2026-05-07 (PR #136).** The flake-prone test was retired entirely as a side-effect of the #78 lock-file rewrite: the new singleton mechanism is OS-lock-based (not state-based), so the prior subprocess-spawn-based test is replaced by `test_run_refuses_when_singleton_lock_is_held` plus three lock-helper unit tests. The new shape acquires the lock from the test process directly — no subprocess, no system-wide PID race, deterministic on every platform.
...
**Status: RESOLVED 2026-05-08 (PR #144).** Framing correction — the body's claim that `sys.executable` is "always a non-empty string in any running Python process" was wrong. Python docs explicitly allow `""` or `None` on embedded interpreters / misconfigured frozen deployments, so the guard had real protective value. Codex review caught the regression after the initial draft of this PR dropped the guard; the merged shape **keeps** the `if not python: raise RuntimeError(...)` arm. The fix reframed the original BACKLOG concern: the docstring's "Raises RuntimeError if no python3 is on PATH and no shim is found" line did not acknowledge the embedded-interpreter / misconfigured-frozen edge case the guard actually covers; the docstring is now explicit about that. New regression test `test_detect_invocation_raises_when_no_python3_and_no_sys_executable` pins the contract by monkeypatching `shutil.which` to return None and `sys.executable` to `""`, asserting `RuntimeError` is raised. Companion fix #50 in the same PR (Windows non-frozen branch via `pythonw.exe`) is unrelated to the guard and ships as planned.
...
**Fifth observation 2026-05-07** in PR #134's Windows-only integration leg, after the percent-escape fix commits (`e289d00` `fix(install): double-escape % in ExecStart...` + `b993593` `docs(backlog): note % specifier escape...`) were pushed. Same shape: `_poll_until(...timeout_s=5.0)` returned False on the negation-semantics assertion. Neither commit touches the daemon dispatch path, the watchdog handler, the rules conflict detector, or any Windows code path — `linux_systemd.py` is Linux-only and not imported on Windows. macOS and Ubuntu legs both passed. The earlier Windows leg on PR #134 (commit `baef791`, before the percent-escape commits) had passed — same code, different runner-load. Reran via `gh run rerun --failed`. Five observations since the PR #40 timeout widening makes the fix shape "kick the can with timeout" definitively wrong; the open question is whether the (B) fix candidate (instrument + diagnose) or (C) fix candidate (replace daemon-driven smoke with unit-level synthetic events) is the right next step.

**Diagnosis attempt 2026-05-07** via candidate (B) instrument-and-diagnose. PR #135 (`chore/34-instrument-daemon-timing`) adds DEBUG-level boundary logs at every stage of the dispatch chain: `_WatchdogHandler.on_any_event` (event arrival), the fast-path `cache.match` decision, `Debouncer.submit` (queue-in) + `Debouncer._run` emit (queue-out, with `dwell` measuring post-deadline starvation), `_dispatch` entry, `RuleCache.reload_file` (lock-held duration), `reconcile_subtree` start/end (walk wall-clock), and per-path `set_ignored` / `clear_ignored` (NTFS ADS write latency). The smoke test sets `DBXIGNORE_LOG_LEVEL=DEBUG` and dumps `daemon.log` to stderr on assertion failure so a failed CI leg captures the per-stage trace. Anchored against test-side `TRACE` lines emitted via `time.monotonic()` so a post-hoc reader can correlate test wall-clock with daemon-internal stage timings. Hypothesis space the instrumentation differentiates: H1 kernel/AV event-delivery latency; H2 debouncer worker thread starving; H3 reconcile walk slow under AV; H4 NTFS ADS write blocked by AV; H5 fast-path `cache.match` race; H6 rule-cache reload lock contention; H7 stale-observer interference from earlier tests.

PR #135 had **six consecutive Windows leg passes** (initial push + 4 reruns + a parallel rerun on the same SHA, durations 37–46s) — the flake did not reproduce in any of them. Inconclusive: 6 runs without failure is consistent with both "the flake's elevated rate from PR #134 was a transient runner-load condition that has since subsided" and "the instrumentation's added DEBUG logging or the test's `DBXIGNORE_LOG_LEVEL=DEBUG` env-var setup happens to break the race window the bug exploits." With a binomial assumption of ~25% per-run failure rate (consistent with the long-run history), the probability of 6 passes in a row is `0.75^6 ≈ 0.18` — not extreme enough to call the flake "fixed" by this PR's incidental changes. The instrumentation is now permanently deployed; the trace capture is opportunistic from here. The next time the flake hits — on this branch's CI matrix or any future PR — the daemon.log dump on stderr will surface the per-stage trace and the H1–H7 mapping resolves it.

**Sixth observation 2026-05-07** in PR #136's Windows leg captured the long-awaited trace. The instrumentation paid off — the captured `daemon.log` shows the initial sweep completed normally (`set_ignored` on `build/` in 35ms, sweep done in 38ms) and the observer logged `watching roots: [...]` at wall-clock 18:31:59,911. **Then nothing for 5+ seconds.** Zero `on_any_event` records, zero debouncer `submit`/`emit` records, zero further `dispatch` or `reconcile_subtree` records. The test wrote the second `.dropboxignore` and `mkdir build/keep/` in this window (TRACE shows both at monotonic 1787.6400, immediately after the first poll succeeded), but the daemon never observed those events.

The trace **rules out all seven H1–H7 hypotheses**: H1 (event-delivery latency) would show `on_any_event` records with high latency — not present at all. H2 (debouncer starvation) would show `submit` records with long `dwell` on emit — also absent. H3/H4 (reconcile/ADS slow) would show long durations — none recorded after the initial sweep. H5 (fast-path race) would show `fast-path: match=False` — not logged. H6 (lock contention) would show long `reload_file` — not present. H7 (stale observer) would show duplicate `on_any_event` records — also not present. The actual cause is a hypothesis the original list didn't include: **`ReadDirectoryChangesW` events are silently dropped by the Windows kernel or the `watchdog` library** on this CI runner under some condition we haven't yet isolated. Not slow — *missing entirely*. No timeout widening would have helped; this confirms candidate (A) (further widening) is definitively the wrong shape and validates the original 2026-05-04 abandonment of that approach.

The captured trace and the H8 ("events are dropped, not delayed") diagnosis suggest the next step is candidate (C): replace the daemon-driven multi-event smoke with a unit-level test that fires synthetic `EventKind` events directly into `_dispatch`, bypassing the watchdog event loop entirely. That preserves coverage of the rule-update + reconcile + marker-write chain while eliminating the kernel-event-delivery dependency that this run proves is intermittently unreliable. The smoke test could be reduced to a minimal "daemon starts + responds to ONE event" assertion (less brittle than the current multi-event scenario). A separate watchdog-only test, possibly Linux-only or with a known-good event source (file-creation in a controlled temp dir), would still cover the watchdog regression surface. Ticket-worthy as a follow-up.

**Status: RESOLVED 2026-05-07 (PR #139).** Candidate (C) landed. New `tests/test_daemon_synthetic_events.py` fires stub watchdog events directly into `daemon._dispatch` against a real `RuleCache` + `FakeMarkers` backend — three tests cover (1) rule-create marks the matched directory, (2) rule-modify with a conflict-detector-dropped negation keeps the child marked AND emits the `masked by` WARNING via `caplog`, (3) DIR_CREATE non-fast-path arm marks the directory. Cross-platform (the chain is platform-agnostic; xattr/ADS marker-backend coverage stays in the existing `test_{linux,macos}_reconcile_smoke.py` and `test_windows_*` suites). The `_stub_event` helper from `tests/test_daemon_dispatch.py` was promoted to `tests/conftest.py:stub_event` so both test files share the same mock shape. `tests/test_daemon_smoke.py` reduced from the multi-event chain to a single-event "daemon starts + reacts to one event" canary with a 10s budget — Windows-only, kept as the live-watchdog smoke. If that single-event variant ever flakes, H8 (Windows kernel silently dropping events) is the cause and the test should be deleted rather than widened further; the assertion message points at the cause.

Touches: `tests/test_daemon_synthetic_events.py` (new); `tests/test_daemon_smoke.py` (reduced); `tests/test_daemon_dispatch.py` (import shared helper); `tests/conftest.py` (promote `stub_event`).
...
**Status: RESOLVED 2026-05-07 (PR #134).** `build_task_xml` now passes `getpass.getuser()`, `str(exe_path)`, and `arguments` through `xml.sax.saxutils.escape` before interpolating each. Three regression tests in `tests/test_install.py` exercise `&`-in-exe-path, `&<>`-in-username (with the username asserted via `xml.etree.ElementTree.findall` so the round-trip through escape+parse is verified), and `&`-in-arguments (defensive, since arguments are interpolated too). All three failed against the un-fixed code.
...
**Status: RESOLVED 2026-05-08 (PR #144).** Took the preferred fix: `_common.detect_invocation` now branches on `sys.platform == "win32"` in the non-frozen path and returns `(Path(sys.executable).with_name("pythonw.exe"), "-m dbxignore daemon")`, intentionally skipping the `shutil.which("dbxignored")` lookup on Windows (any shim would still launch `python.exe` with a console). `install/windows_task.py` collapsed to `from dbxignore.install._common import detect_invocation as detect_invocation` (re-export silences strict-mypy implicit-reexport). Four redundant frozen-branch tests in `tests/test_install.py` removed; a new `test_detect_invocation_returns_pythonw_on_windows` in `tests/test_install_common.py` mocks `sys.platform = "win32"` and pins the pythonw.exe selection. The two existing non-frozen tests in `test_install_common.py` (`falls_back_to_python_module`, `uses_path_shim_when_present`) gained `monkeypatch.setattr(sys, "platform", "linux")` so they exercise the Linux/macOS arm rather than the new Windows short-circuit.
...
## 53. Initial-sweep wall-clock on a fresh tree (no existing markers) — 49.62s on 27k dirs

**Candidate 1 (ready-before-sweep) RESOLVED 2026-05-08 (PR #162).** **Candidate 3 (per-subdir worker fan-out) RESOLVED 2026-05-09 (PR #183).** Candidate 2 (persisted sweep-complete hint) remains open; the body below is preserved as future-reference. After #162 the user-visible systemd-startup pause was removed (sweep wall-clock unchanged); after #183 the sweep itself parallelizes across each root's top-level subdirs (`os.cpu_count()`-capped via a single `ThreadPoolExecutor`), so wall-clock improves on multi-subdir trees too.

**Reframed 2026-05-08 in PR #157** after the original fix candidate was shown to already be implemented (see "Original framing" below). Surfaced 2026-05-03 during VPS testing — Ubuntu 24.04, Python 3.14, 27,000-directory personal Dropbox tree, journalctl shows the initial sweep took 49.62s. The manual test's daemon-startup poll initially timed out at 30s; the test was fixed by raising the poll to 180s, but the underlying app cost is the real concern.
...
The user-visible symptom: `dbxignored.run()` performs the initial sweep BEFORE writing `state.json` with the daemon's pid + create_time. The systemd unit start (and any external poller, including the manual-test scripts) wait on `state.json` existing as the readiness signal. On a 27k-dir fresh install, that's a ~50s pause between `systemctl --user start dbxignore` returning and `dbxignore status` reporting `running`.

**Fix candidates** (the original "skip on marker-present + match-still-positive" candidate is dropped — it described the existing pruning):

- **Mark the daemon ready before the initial sweep finishes.** **DONE in PR #162.** Restructure `daemon.run()` so the singleton lock + observer schedule + first `state.json` write happen first, then the initial sweep runs in a worker thread. The systemd-startup pause symptom goes away even though wall-clock is unchanged. ~30 LOC plus an audit of any state.json reader that assumed "exists ⇒ initial sweep complete." Lowest blast radius; addresses the user-visible pain without algorithmic risk.
...
- **Concurrent per-top-level-subdir workers.** **DONE in PR #183.** Today `_sweep_once` parallelizes at the per-root level via `ThreadPoolExecutor` (one worker per root). A single 27k-dir root has ~10–50 top-level subdirs; spawning a worker per top-level dir would parallelize the walk further. Bounded by CPU count and disk-contention; expect a 2–4x wall-clock reduction on common SSD-backed installs. ~40 LOC plus thread-safety review of `RuleCache._applicable` traversal under finer-grained concurrency (the existing per-root parallelism already accepted slight ancestor-view drift per CLAUDE.md).

- **Defer.** The initial-sweep cost is a one-time per-fresh-install pain. Workaround documented today: run `dbxignore apply` synchronously before `dbxignored install` so the markers exist before the daemon's first sweep, which then prunes correctly.

**Urgency:** medium-low. Fundamental to evaluating an N-dir tree once; reproduces only on fresh installs of personal-account-sized trees. Hourly steady-state sweeps are unaffected. With candidate 1 done, the user-visible systemd-startup symptom is fixed; the remaining wall-clock concern is purely "the sweep itself takes 50s" and is the trigger for candidates 2 + 3.

Touches: `src/dbxignore/daemon.py` (candidates 1 + 3); `src/dbxignore/state.py` (candidate 2's hint field); `src/dbxignore/reconcile.py` (candidate 3's worker fan-out); `tests/test_reconcile_basic.py` already documents the steady-state pruning contract via `test_does_not_descend_into_marked_subtree` — new tests would cover the chosen candidate's incremental contract.

### Original framing (2026-05-03 → 2026-05-08, dropped)

The original body proposed adding a "skip on marker-present + match-still-positive" check before recursing — read the marker xattr, prune if present and match still confirms. PR #157 verified that `_reconcile_path` already returns `True` for this case (the no-mutation tail), and `dirnames[:]` already filters those out. The behavior the original framing proposed already exists. Closed-as-misframed precedents: #59 (headline already implemented), #62 (model-mismatch), #75 (framing-correction during /simplify). Reframe rather than close because the underlying perf pain (49.62s) is real — just not addressable by the originally-proposed fix.
...
**Status: RESOLVED 2026-05-07 (PR #133).** Fix candidate 1 applied: dropped the `if discovered and ...` guard, kept the inner `find_containing(...) is None` check. `find_containing(any, [])` returns `None`, so the warning fires with the same wording when no roots are discovered. Regression test `test_generate_warns_when_no_roots_discovered` in `tests/test_cli_generate.py` pins the contract.
...
**Status: RESOLVED 2026-05-07 (PR #133).** Fix candidate 1 applied: `_apply_from_gitignore` now calls `cache.load_external(source, mount_at, log_warnings=False)`, restoring stderr parity with the regular `apply` path. Regression test `test_apply_from_gitignore_suppresses_rules_conflict_warnings` in `tests/test_cli_apply.py` asserts no `dbxignore.rules` WARNING records are produced when a conflicted gitignore is applied (mirrors the precedent at `tests/test_cli_status_list_explain.py:307`).
...
**Status: RESOLVED 2026-05-08 (PR #146).** Took fix candidate (2) — cache the count in `state.json`. Added `State.last_sweep_conflicts: int = 0` (with decode-tolerance for pre-#68 state.json files via `.get("last_sweep_conflicts", 0)`); `daemon._sweep_once` writes `len(cache.conflicts())` after the load_root pass (free — `cache.conflicts()` reads the already-computed `_conflicts` list from `_recompute_conflicts`); `cli.status` now branches on `--summary`: summary path reads `s.last_sweep_conflicts if s else 0` and skips `_load_cache` entirely; human path keeps the live walk because it shows the actual conflict details, not just the count. Trade-off: the count is from the last daemon sweep, same staleness lineage as `last_sweep_marked` / `cleared` / `errors`. The format string `state=... conflicts=N` is unchanged so the public API contract holds. Three new tests: state round-trip with non-default conflicts count, decode-tolerance for missing field, and a `status --summary` test that monkeypatches `_load_cache` to raise — pins the perf invariant against future regressions.
...
**Status: RESOLVED 2026-05-07 (local-only — `.claude/settings.json` is gitignored at `.gitignore:3`, so the fix lives on the maintainer's machine and is not part of the published repo).** Two more fired triggers in this session (PR #139 and PR #140 pre-flight `commit-check` loops) made fix candidate (B) the right pick. The hook's `command` field now reads `tool_input.command` from stdin via `printf '%s' "$INPUT" | jq -r '.tool_input.command // empty'` and short-circuits with `exit 0` unless the command begins with the literal `gh pr create`. The original `if: "Bash(gh pr create*)"` field is left in place as belt-and-suspenders (whether or not it's actually being honored is now irrelevant). Verified with six test cases covering the over-fire shapes (`for sha in ...; commit-check`, `gh api`, `gh pr edit`, `gh --version`) and the actual-fire cases (`gh pr create` with marker present → silent; without marker → deny JSON emitted). The `printf '%s'` over `echo` choice is defensive against POSIX `echo`'s implementation-defined backslash handling — the JSON payload contains escape sequences that could be corrupted otherwise.
...
**Status: RESOLVED 2026-05-08 (PR #149).** Took fix candidate (1) — conservative drop. New `_find_masking_directory_include` helper in `rules_conflicts.py` walks earlier entries and returns any include whose raw text ends in `/` (covers literal-prefix `build/` AND glob-prefix `**/foo/`, excludes children-only `build/*` and file-level `*.log`). `_detect_conflicts` gains a glob-prefix arm: when `literal_prefix(raw) is None` AND the negation is directory-targeting (raw ends in `/`), look up a directory-marking include in the prior sequence and flag the conflict. File-level glob-prefix negations (`!**/important.log`) still skip — file-level rules don't propagate via Dropbox inheritance. Three tests adjusted: existing shim-based `test_detect_skips_glob_prefix_negation` renamed and flipped to assert the conflict; existing real-pathspec `test_rulecache_no_conflict_for_glob_prefix_negation` (added in PR #142) renamed and flipped similarly; three new tests pin the don't-flag cases (file-level negation under directory include, glob-prefix dir negation with no directory include, glob-prefix dir negation alone). Marker behavior is unchanged — the bug surface was always diagnostic-only (`status` / `explain` / the conflict WARNING) and that's where the fix lands.
...
**Status: RESOLVED 2026-05-08 (PR #148).** Took fix candidate (1) — structured tuple key. New `DebounceRole = Literal["single", "moved-out", "moved-into"]` and `DebounceKey = tuple[DebounceRole, str]` types in `debounce.py`. `Debouncer._pending`, `Debouncer.submit`, and the `on_emit` callback signature now use `DebounceKey`; `daemon._classify` returns the tuple shape with `role` set to one of three values: `"moved-out"` for moved events with src=rule_file (the previously-undisambiguated case), `"moved-into"` for moved events with dest=rule_file (replaces the `moved-into:` string prefix from PR #120), and `"single"` for everything else (create/modify/delete on rule files plus all DIR_CREATE / OTHER events). New regression test `test_classify_moved_out_and_single_same_path_have_distinct_roles` pins the moved-out vs single disambiguation that the body called out as the remaining first-vs-second-shape collision. Existing tests in `test_debounce.py` and the two `test_daemon_dispatch.py` key-equality tests switched from string to tuple comparisons; the `moved_out_and_moved_into_same_path_have_distinct_keys` test still passes (now via tuple inequality rather than string-prefix difference).
...
**Status: RESOLVED 2026-05-07 (PR #136).** Fix candidate 1 (OS lock file via `fcntl.flock` / `msvcrt.locking`) applied. New `_acquire_singleton_lock()` helper in `daemon.py` opens `user_state_dir() / "daemon.lock"` and applies a non-blocking exclusive lock; held for the daemon's lifetime, kernel-released on process exit. The prior state-based check (read state.json → `_is_other_live_daemon`) is removed in favor of the lock; on contention, a best-effort state.json read recovers the existing daemon's PID for a "daemon already running (pid=X)" error. Three new helper-level unit tests cover fresh-acquire, contention-returns-None, and after-release-succeeds. The existing `test_run_refuses_when_another_pid_is_alive` (subprocess-spawn) is replaced with `test_run_refuses_when_singleton_lock_is_held` (lock acquired from test thread) — closes #14 as a side effect.
...
**Status: RESOLVED 2026-05-07 (PR #136).** Fix candidate 1 (persist `psutil.Process.create_time()` alongside `daemon_pid`) applied. New `daemon_create_time: float | None` field on `State`, encoded/decoded in `state.json` with decode-tolerance for v0.4.x files predating the field. `is_daemon_alive(pid, create_time=None)` extends to compare `psutil.Process(pid).create_time()` to the caller-supplied value within 1ms slack when `create_time` is provided; falls back to the legacy substring-name check when None or psutil is missing. `daemon_is_running` wrapper auto-forwards `state_obj.daemon_create_time`. Daemon captures its own `create_time` at startup and threads it through `_sweep_once` to `state.write` so future startup checks see the correct (pid, create_time) pair. Three new test cases cover create-time match, mismatch (recycled-PID-with-different-create-time), and None-fallback paths.
...
**Status: RESOLVED 2026-05-07 (PR #134).** Fix candidate 1 (Quote + escape) applied as a conditional: paths containing whitespace, `"`, or `\` are wrapped in double quotes with C-style escapes; bare paths stay unquoted to match the on-disk shape stock distro installs have today. Additionally, `%` is doubled to `%%` regardless of quoting because systemd expands `%X` specifiers (`%T` → `/tmp`, `%h` → home, etc.) in `ExecStart` at unit-load time — without the `%%` escape, an install path like `/home/me/100% Tools/dbxignored` is silently rewritten by the specifier expander and the unit points at the wrong binary. A literal `$` embedded in the path is **not** escaped: per `man systemd.service` "Command Lines", the bare `$VAR` form is only expanded when it is the entire argument (and the docs note "the first argument may not be a variable"), so a `$` mid-path is passed through. The `${VAR}` brace form IS expanded mid-string, but is rare enough in real install paths to defer — and the right escape for that form is a more careful `${` → `$${` substitution, not a global `$` → `$$`. New helper `_quote_exec_start_path(exe_path)` is the seam. The pre-existing `_escape_systemd_env_value` was renamed to `_escape_systemd_quoted_string` since the same C-style escape rules apply to both `Environment="KEY=VALUE"` and `ExecStart="/path with space"`; the helper is now reused from both call sites. Eight regression tests in `tests/test_linux_systemd.py` cover the whitespace, whitespace-with-arguments, escape-needing-chars, percent-with-whitespace, percent-bare, dollar-passthrough-quoted, dollar-passthrough-bare, and unquoted-bare-path branches. The percent-escape branch was added in response to a Codex bot review on PR #134; the dollar-passthrough branches were added after a follow-up Codex review correctly pointed out that `$VAR` in a mid-path position is not expanded by systemd, and an earlier `$$` escape was reverted. Validate-and-refuse (candidate 2) was rejected because quoting is the systemd-idiomatic answer and works in all cases the user might reasonably hit.
...
**Status: RESOLVED 2026-05-08 (PR #159, Dependabot).** Self-resolved by Dependabot's `ci(deps): bump actions/checkout from 4.3.1 to 6.0.2` PR, merged 2026-05-08T12:55:59Z — 38 minutes after this item was filed in PR #156. Dependabot bumped both lagging files (`claude.yml`, `claude-code-review.yml`) directly past v5 to v6.0.2; all six workflow files now pin the same SHA `de0fac2e...` tagged `# v6.0.2`. The setup-uv v6/v7 arm of this item went moot earlier when PR #163 retired `codex-followup.yml`. Closure mechanism matches the body's "may eventually self-resolve" prediction in the Urgency paragraph. Backfilled 2026-05-09 (PR #178) after the closure went unrecorded for a day — discovered while prepping a v4→v5 bump that turned out to be a no-op.
...
- **Switch the push step to a PAT or separate GitHub App installation token with workflow/write access.** `workflows` is not a valid GITHUB_TOKEN permission key — it does not appear in GitHub's documented `jobs.<job_id>.permissions` scope list, so adding `workflows: write` under `permissions:` would be silently invalid (or fail workflow schema validation), not grant the missing permission. The actual gate is that GitHub refuses to let the default GITHUB_TOKEN push commits that modify files under `.github/workflows/` regardless of what scopes are listed; only a PAT with `workflow` scope or a GitHub App installation token with the `workflows` write permission set at the App level can do so. Eliminates the limitation entirely but expands the security boundary: a malicious PR with a poisoned Codex comment could escalate via the bot to modify CI/CD. Defensible if the maintainer values the autonomy more than the guardrail; the credential type and scope should be documented inline in the workflow if taken.
...
Touches: `.github/workflows/codex-followup.yml` — the `prompt:` block under "Your job" if option 1; the push step and secrets/token wiring if option 2 (PAT or GitHub App token, not a `permissions:` key).

**Status: RESOLVED 2026-05-08 (PR #163).** Workflow retired entirely rather than fix the gap. PR #162's six-finding Codex review pass demonstrated that auto-fix budget exhaustion (3 commits) cuts in too early relative to typical Codex review depth on a non-trivial daemon refactor. Past the budget, the bot's escalation replies were duplicative of the maintainer's own session-driven fixes — same finding got two analyses (one from the bot, one from the maintainer) and one fix. With the workflow retired, Codex review still works (manual `@codex review` summon stays available) but the reply path is maintainer-only. The choice is between "richer auto-fix on small PRs, redundant noise on larger ones" and "consistent maintainer-in-the-loop for everything"; chose the latter. If the auto-fix-on-small-PRs value ever feels missed, the workflow file is recoverable from `git show <pre-PR-163>:.github/workflows/codex-followup.yml`.
...
Concretely: a 100k-directory tree with 5 `.dropboxignore` files at top level. SIGTERM arrives during phase 1 of the initial sweep. The `rglob` is mid-traversal of the bottom of the tree. Cancellation isn't observed until the next yield, which may not come for tens of seconds (rglob has to traverse every dir to check for the filename). Total shutdown latency: bounded by rglob completion time + cooperative-cancellation observation lag.

Operationally bounded by systemd's `TimeoutStopSec=90s` default — beyond that, SIGKILL terminates the daemon. Not a correctness break (state.json's atomic temp+replace, marker writes are OS-atomic), but exceeds the "cooperative shutdown returns within ~1s" property the design implicitly suggested.
...
- **Defer.** The 90s systemd backstop is operationally acceptable. Most users don't notice unless they're on a very large tree AND restarting frequently.

**Urgency:** low. Bounded by systemd's default TimeoutStopSec; no correctness regression. Bundle with the next `daemon.py` or `rules.py` edit if the manual-walk option is taken; the alternative architectural fix is large enough to warrant its own design doc.

Touches: `src/dbxignore/rules.py` (`load_root`'s rglob loop) for option 1; `src/dbxignore/daemon.py` (`run`'s shutdown sequence) + `src/dbxignore/state.py` (`is_daemon_alive`'s contract) for option 2.
...
The fix in commit `fde50c5` polls until `state.json`'s `daemon_pid` is non-null AND different from the pre-uninstall value (`$uninstallPid`). The fix is correct under the conditions Codex described; what's missing is an explicit regression pin. If a future refactor reintroduces the read-first-iteration shape, the test would still pass on a fast host (where the new daemon happens to write `state.json` before the first poll iteration runs) and fail intermittently on slow hosts — exactly the flake mode that's hardest to triage.

**Fix candidates:**

- **Add an explicit `$purgePid -ne $uninstallPid` assertion to `Test-Uninstall`.** ~5 LOC: `if ($purgePid -and $uninstallPid -and ($purgePid -eq $uninstallPid)) { Write-Fail "regression: post-reinstall PID poll returned stale pid" }`. The assertion gracefully degrades when either pid is null (state.json unavailable). Cheapest defensive layer.

- **Extract the polling logic into a helper and unit-test it.** Mock `Get-Content` to return a sequence of state.json contents (stale → fresh) and verify the helper polls past the stale value. Requires the polling block to be a function (currently inlined). Higher cost; better long-term defense; useful if the same shape is needed elsewhere.

- **Defer.** Item #87 will eventually land and remove the entire test-level workaround (PID dance, Wait-Process, polling block — all gone once `uninstall_task()` is synchronous). The regression pin has a self-limiting lifespan tied to #87's resolution.

**Urgency:** low. The fix in `fde50c5` is sound under the conditions Codex described; the regression pin is belt-and-braces against a future refactor. Bundle with the next `manual-test-windows.ps1` edit if convenient. Wait for #87 to land if not.

Touches: `scripts/manual-test-windows.ps1` (`Test-Uninstall` body) for option 1.

---

## 89. Manual-test scripts have no deterministic way to exercise the long-sweep arms

**Status: RESOLVED 2026-05-09 (PR #175).** Took fix candidate (1) but with a framing correction: a **marker file** at `state.user_state_dir() / "_test_slow_sweep"` rather than the prescribed `DBXIGNORE_TEST_SLOW_SWEEP_S` env var. The body's env-var path was ~10 LOC daemon-side as predicted, but the install-layer surface is asymmetric — Linux/macOS would need to extend `_FORWARDED_ENV_VARS` (today scoped to `DBXIGNORE_ROOT`; other `DBXIGNORE_*` tuning vars are set via unit/plist drop-ins, not via env forwarding), and Windows Task Scheduler XML has no per-task env-var forwarding mechanism at all (would need ~30 LOC of wrapper-script logic). The marker-file path keeps the install layer untouched and works cross-platform-uniformly. `daemon._slow_sweep_pad_seconds()` reads + parses + WARNs (mirrors `_timeouts_from_env` validation shape); call site at top of `_initial_sweep_worker` uses `stop_event.wait(pad_s)` so shutdown stays prompt mid-pad. `cli._purge_local_state` cleans the marker on `uninstall --purge` so a stale marker can't silently pad future installs. All three manual-test scripts seed the marker before `dbxignore install` (15s pad — picked to deterministically catch 5a's 5-iteration `state=starting` poll and let 5f's 180s poll observe the transition) and remove it at end of phase 5; the Linux script's existing `trap cleanup EXIT` arm catches phase-5 aborts, and the macOS/Windows scripts include the cleanup in `phase_cleanup` (happy-path-only — those scripts have no trap-equivalent today). 16 new tests in `tests/test_daemon_slow_sweep.py` cover the parse/validation/integration matrix (12 unit + 4 worker-integration; the 4th worker-integration test was added by PR #179 closing item #91), plus one `test_purge_removes_slow_sweep_marker` in `tests/test_install.py`. **Three Codex P2 + one P3 catches in the same PR.** Two share a daemon-side shape — an unhandled exception type that propagates out of `_slow_sweep_pad_seconds`, escapes the worker thread (the helper call is BEFORE `_initial_sweep_worker`'s `try` block), and strands the daemon in `state=starting` forever: (1) `float("inf")`/`nan` parse successfully and slipped past the `< 0` / `> 0` arms; `stop_event.wait(inf)` then raises `OverflowError`. Fixed via `not math.isfinite(value) or value > threading.TIMEOUT_MAX` rejection (Windows's `TIMEOUT_MAX` is ~4.3M seconds, much smaller than POSIX's, so a Linux-picked value can over-shoot Windows). (2) Non-UTF-8 marker contents raise `UnicodeDecodeError`, which derives from `ValueError`, not `OSError` — surfaces when a stale marker was written in a different encoding (Windows PS 5.1 `Set-Content` defaults to UTF-16, corrupt/binary edits). Fixed by extending the OSError catch to `(OSError, UnicodeDecodeError)`. The third P2 was a script-orchestration bug: 5b/5c/5d created watch files within the 15s pad window, but `cache.load_root` runs inside `_initial_sweep_worker` (item #53 + daemon.py:638) — empty cache during the pad meant watchdog events dispatched against `match()=False` and the rule didn't apply. Fixed by adding a 5a-post `state=running` gate (up to 180s) before 5b in all three scripts; the gate also makes 5b/5c/5d deterministic on real ~/Dropbox trees where a slow sweep could race the existing 8-second window. The P3 was a Linux-script-only path divergence — hardcoded `$HOME/.local/state/dbxignore` for the marker write missed when `XDG_STATE_HOME` is set; replaced with a `DBXIGNORE_STATE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/dbxignore"` variable mirroring `state.user_state_dir()`. Four regression tests on the daemon-side catches: `inf`, `nan`, `TIMEOUT_MAX + 1.0`, and UTF-16-BOM bytes.

**Surfaced 2026-05-09 in PR #169 (Codex P2 review, second-pass suggestion to "add a Windows manual case with a slow initial sweep").**

PR #169 added Phase 5/6 cases sized for the ~50s sweep window observed on a personal-account `~/Dropbox` tree (~27k dirs): 5a captures `state=starting` while the initial sweep runs, 5f polls up to 180s for `state=running`, 6a absorbs the Windows schtasks fire-and-forget gap, 6b waits for the re-installed daemon to write its own `state.json`. 5f's 180s ceiling is explicitly headroom for ~100k dirs.

The arms exist; what's missing is a deterministic way to exercise them. On a small test tree the initial sweep finishes faster than any timing-sensitive case can observe the transient state — 5a falls through to its `note` path (informational, not failing), 5f converges on iteration 1 with no real test of the long-sweep contract, and 6a/6b's poll loops succeed on iteration 1 too. The arms are correctly written and sized; the manual scripts just don't deterministically reach the long-sweep regime unless the tester happens to have a real ~/Dropbox tree with tens of thousands of dirs.

**Fix candidates:**

- **Daemon-side `DBXIGNORE_TEST_SLOW_SWEEP_S` env var.** Add a guarded `time.sleep(N)` (or per-directory sleep) in `_initial_sweep_worker` when the var is set. Manual-test scripts export the var before `dbxignore install` and observe deterministic long-sweep timing on any tree size. Test-only feature in production code; scope it tightly (off by default, ignored unless `os.environ.get(...)` is set, log a WARNING when honored so it's never silently active). ~10 LOC daemon-side + ~40 LOC per-script for a new "slow-sweep" Phase 5 variant.

- **Synthetic large tree.** Have the manual-test scripts pre-create thousands of empty subdirs under the test root before Phase 5, so the daemon's initial sweep takes the same wall-clock time as a real big Dropbox install. Most faithful but slowest. ~50 LOC of setup + correspondingly slow runtime; cleanup needs care.

- **Defer.** Leave the `note` fall-through as the documented small-tree caveat. Honest about the coverage gap; not closing it. PR #169 lands in this state.

**Urgency:** low. The arms are correct; what's missing is deterministic exercise. Independent of item #87 — Phase 5 timing tests would still benefit from determinism even after #87 retires the Windows-specific 6a/6b PID dance. Bundle with the next manual-test-script edit if convenient.

Touches: `src/dbxignore/daemon.py` (`_initial_sweep_worker`) for option 1; `scripts/manual-test-{ubuntu-vps,macos}.sh` + `scripts/manual-test-windows.ps1` (Phase 5 setup) for both options 1 and 2.

---

## 90. Linux manual-test script has hardcoded `$HOME/.local/state/dbxignore` paths that miss `XDG_STATE_HOME`

**Status: RESOLVED 2026-05-09 (PR #177).** Replaced all four hardcoded `$HOME/.local/state/dbxignore/` references in `scripts/manual-test-ubuntu-vps.sh` (lines 442, 511, 741, 745) with `$DBXIGNORE_STATE_DIR`, the `${XDG_STATE_HOME:-$HOME/.local/state}/dbxignore` variable PR #175 introduced for the slow-sweep marker site only. Mechanical 4-line script change; `bash -n` parse-clean. Linux-only by design — macOS and Windows scripts have no XDG-equivalent and were correctly left alone. **Codex P2 catch in the same PR** revealed a deeper production gap the script consolidation exposed: `install/linux_systemd.py:_FORWARDED_ENV_VARS` only carried `DBXIGNORE_ROOT`, so the systemd-launched daemon never inherited `XDG_STATE_HOME` from the install caller's shell. Result: a user with `XDG_STATE_HOME` set ran their own tools (`dbxignore status`, etc.) probing one path while the daemon wrote state to another (the fallback `~/.local/state/dbxignore`). Fixed by adding `XDG_STATE_HOME` to `_FORWARDED_ENV_VARS` so `install_unit()` propagates it into the unit's `[Service]` Environment block. Two regression tests pin the contract (forwarding when set, omission when unset); three pre-existing tests gained `monkeypatch.delenv("XDG_STATE_HOME", raising=False)` so a host-set value can't leak in. **Codex P3 catch in the same PR**: systemd performs specifier expansion inside `Environment=` values too (per `systemd.exec(5)`), so a literal `%` in a forwarded `XDG_STATE_HOME` (e.g. `/home/me/100%state`) would be silently rewritten by the specifier expander — same shape as item #82's ExecStart `%` doubling, asymmetric until now. Fixed by doubling `%` → `%%` before the C-style quoted-string escape at the `Environment=` line construction; one new test (`test_unit_content_escapes_percent_in_environment_value`) mirrors the existing ExecStart percent tests.
...
macOS and Windows scripts are unaffected — `~/Library/Application Support/dbxignore` has no XDG-equivalent on darwin, and Windows's `%LOCALAPPDATA%` is what `state.user_state_dir()` reads directly.

**Fix candidates:**

- **Replace all four references with `$DBXIGNORE_STATE_DIR`.** ~10 LOC, mechanical. Same shape as the slow-sweep marker consolidation in PR #175. No shell-script test infrastructure exists in this project (per #89's framing), so verification is via `bash -n` parse-check + manual inspection.
...
**Status: RESOLVED 2026-05-11 (PR #203).** `state._read_at()` broadened to catch generic `OSError` (permission, lock, cloud-placeholder, transient unavailability) on top of the existing `JSONDecodeError`/`KeyError`/`TypeError`/`ValueError` arm; logs `WARNING` and returns `None` so `dbxignore status`, `clear`'s daemon-alive guard, `daemon.run`'s legacy-state migration path, and Windows uninstall's daemon-exit wait all degrade gracefully rather than crash. Dropped the pre-`exists()` LBYL check in favor of catching `FileNotFoundError` from `read_text()` itself (closes the exists/read TOCTOU window). `cli.clear`'s daemon-alive guard additionally fails closed (exit 2 with `error: state.json at <path> is present but unreadable; daemon liveness is unknown.`) rather than treating "unknown" as "no daemon" and racing the live daemon's marker re-apply; `--force` overrides. Shipped in v0.5.0.
...
**Status: RESOLVED 2026-05-11 (PR #204).** `uninstall --purge` now accumulates per-path marker errors as `(path, operation, message)` tuples (operation tagged `read` during `markers.is_ignored` or `clear` during `markers.clear_ignored`), prints up to 10 on stderr in the form `Could not fully clear markers (N errors):` + `  <op> failed on <path>: <message>` with an `... and X more.` tail when applicable, and exits 2 on any failure. Local-state cleanup (`state.json`, daemon logs, Linux systemd drop-ins) still runs after a partial marker failure — only the exit code and stderr report are new. `FileNotFoundError` from a path that vanished between `os.walk` and the marker call is treated as a non-error (mirrors `reconcile._reconcile_path`'s read-arm vanished-path treatment). Shipped in v0.5.0.
...
**Status: RESOLVED 2026-05-11 (PR #207).** `rules.append_rule()` and `rules.remove_rule()` now use `tempfile.mkstemp(dir=rule_file.parent, prefix=..., suffix=...)` so each writer picks a unique sibling temp file; two concurrent CLI invocations, an editor's atomic-save backup, or a user-created `.dropboxignore.tmp` cannot collide on the path. The same PR also captures the existing file's POSIX mode (or `0o666 & ~umask` for new files) and `os.chmod`s the temp before the replace, so shared-workflow setups keep their group-readable mode (`mkstemp` creates at `0o600` by default; without the chmod, `os.replace` would silently relock group-readable `.dropboxignore` files to user-only after the first `ignore`/`unignore` invocation). The lost-update window (two readers, two writers, replace race) remains; an advisory lock is deferred until a concrete failure shows. Shipped in v0.5.0.
...
**Status: RESOLVED 2026-05-11 (PR #206).** The cache-invalidation gate switched from `(mtime_ns, size)` stat values to a blake2b-128 hash of the file's bytes. Same heuristic-vs-correctness trade git's `core.checkStat` and rsync's default mode make: missed when an editor preserves mtime AND the new content is the same size, caught cleanly by the hash. The expensive part of the load (pathspec compile) is still skipped when content is unchanged, so the optimization that the `(mtime, size)` shortcut was originally meant to provide is preserved. Shipped in v0.5.0.
...
**Status: RESOLVED 2026-05-11 (PR #205).** All path-taking verbs now refuse paths where `..` follows a symlinked component. The validator's sticky-flag scan walks the un-normalized absolute path's segments, tracks `seen_symlink` via `os.path.islink(prefix)`, and exits 2 on the first `..` that follows a symlinked prefix. A `..` **before** any symlink remains accepted (it cancels a regular segment, no FS-divergence). The guard fires only when the lexical interpretation will be used — out-of-Dropbox alias paths that fall through to the existing `path.resolve()` fallback are unaffected (the resolve fallback is filesystem-true and unambiguous). Took fix candidate (1) — the preferred loud refusal of ambiguous shapes, matching the symlinked-ancestor guard's spirit. Shipped in v0.5.0.
...
**Status: RESOLVED 2026-05-14 (PR #247).** Took the first fix candidate. `format_literal_rule` now calls `_is_real_dir(target)` instead of the inline `target.is_dir() and not target.is_symlink()` form — all three call sites (`cache.match`, `cache.explain`, `format_literal_rule`) share the one-`lstat()` helper. `_is_real_dir`'s docstring drops the "format_literal_rule still uses the two-call form" parenthetical; `format_literal_rule`'s docstring step 5 now says "real directory (not a symlink)". For the six MagicMock-based tests in `test_rules_format_literal_rule.py`: the three dir-case tests (`test_meta_char_escaping_in_segment`, `test_question_mark_and_brackets_escaped`, `test_literal_backslash_escaped_in_segment`) now stub `target.lstat.return_value.st_mode = stat.S_IFDIR` instead of `is_dir`/`is_symlink`; the three whitespace-rejection tests raise `ValueError` before the dir check is ever reached, so their `is_dir`/`is_symlink` stubs were dead setup and are simply dropped.
...
**Status: RESOLVED 2026-05-14 (PR #247).** Took the first fix candidate. `symlink_capable` moved from `test_cli_symlink_path_args.py` to `tests/conftest.py` (placed next to the sibling `require_case_sensitive_fs` runtime-probe fixture; `conftest.py` gained `import os`). The "~10 tests" estimate in this item's body was high — only the call sites with the reason "Windows symlink creation requires admin privileges" genuinely fit: three in `test_cli_ignore.py` (`test_ignore_rejects_path_under_symlinked_ancestor`, `test_ignore_accepts_path_via_outside_dropbox_alias`, `test_unignore_accepts_broken_symlink`) and two in `test_rules_basic.py` (`test_match_treats_symlink_to_dir_as_leaf_not_directory`, `test_explain_treats_symlink_to_dir_as_leaf_not_directory`). The other `skipif(win32)` sites in those files are filename-restriction skips (forbidden `*`/newline/trailing-space) or platform-behavioral (`!= "darwin"` / `!= "linux"`), not symlink-capability gates, so they stay. Each of the five tests dropped its `@pytest.mark.skipif` decorator and added `symlink_capable: None` as a fixture parameter. Verified on a Windows host with Developer Mode enabled: all five now run (and pass) instead of skipping — the portable suite went from 702 passed / 38 skipped to 707 passed / 33 skipped on Windows.
...
**Status: RESOLVED 2026-05-12 (PR #234).** Default CLI log level lowered from `INFO` to `WARNING`. `--verbose` / `-v` is now a counted click flag (was `is_flag=True`): `-v` selects `INFO`; new `-vv` form selects `DEBUG` (preserves DEBUG access for power users). New helper `cli._verbosity_to_level(verbose: int) -> int` maps the counted value to a `logging` level. `daemon_main` gets the same change for symmetry; its `basicConfig` is briefly active before `daemon.run()`'s `_configured_logging()` takes over via `DBXIGNORE_LOG_LEVEL`. No `logger.info` → `click.echo` conversions needed in the audit step — `cli.py`'s existing `click.echo` summary lines already cover user-facing confirmation; install-backend `logger.info` calls (e.g. "Wrote systemd user unit to …", "Bootstrapped …") become operator-visible via `-v`. README updated (DBXIGNORE_LOG_LEVEL table row's CLI parenthetical + Log levels section; INFO row annotated "(daemon default)", WARNING row annotated "(CLI default)"). CHANGELOG entry under `[Unreleased]` > Changed, marked **Breaking** since `--verbose` semantics shift. Five new tests in `tests/test_cli_entrypoints.py` cover the helper's four paths.
...
**Status: RESOLVED 2026-05-12 (PR #227).**
...
**Status: RESOLVED 2026-05-12 (PR #230).**
...
**Status: RESOLVED 2026-05-13 (PR #242).**
...
## 120. Single-binary AttachConsole approach (#30 / PR #238) defeated by PowerShell async-foreground behavior

**Status: RESOLVED 2026-05-13 (PR #239).** Reverted to the well-known `python.exe` / `pythonw.exe` dual-binary pattern: `dbxignore.exe` rebuilt as `console=True` (CLI), new `dbxignorew.exe` built as `console=False` (Task Scheduler daemon entry + Explorer shell-verb helper). Deleted `src/dbxignore/_windows_console.py` (~175 LOC AttachConsole subsystem) and reverted `src/dbxignore/__main__.py` to a plain `cli.main()` wrapper. `_windows_dialogs.should_use_gui_dialogs` switched from `sys.stdout is None` to `ctypes.windll.kernel32.GetConsoleWindow() == 0` so the MessageBox routing in shell-verb invocations is reliable on the GUI helper. `install/_common.detect_invocation` and `detect_cli_invocation` return the `dbxignorew.exe` sibling on the Windows frozen path; `state.is_daemon_alive` accepts both `dbxignore.exe` and `dbxignorew.exe` as valid daemon process names. Release workflow builds both specs, smoke-tests both with `--help` AND `--version`, uploads both as Release assets. Side-effect: `copy_metadata("dbxignore")` added to both Windows + macOS PyInstaller specs to fix a latent `--version` `RuntimeError` that PR #238's GUI-subsystem build surfaced as a traceback dialog (`click.version_option(package_name="dbxignore")` reads `importlib.metadata` which had no dist-info bundled before this fix; v0.5.x had the same latent bug masked by its console-subsystem traceback).

**Post-mortem.** PR #238's single-binary AttachConsole approach worked end-to-end on cmd.exe but failed on PowerShell: PowerShell does not wait synchronously on GUI-subsystem foreground processes, so `dbxignore --help` from a PowerShell prompt appeared to print nothing (the shell prompt redraws before the binary's stdout flushes). `Start-Process -Wait -NoNewWindow` also returned nothing in manual testing — even the most forced-sync PowerShell invocation form couldn't recover the output. The PE subsystem byte is decided at link time, not runtime; no in-process trick fully reconciles "terminal-fluent" and "silent at logon" on Windows. Python solved this with two binaries 25 years ago.

**Lesson:** clever AttachConsole gymnastics can't paper over shell-process-waiting semantics. When two requirements pull on the same link-time bit, the answer is two link products.
...
**Status: RESOLVED 2026-05-14 (PR #249).** Closed by the DBXIGNORE_TEST_FAIL_* meta-fix — `MARKER_READ` fail point + manual-test case 4u/4v.
...
**Status: RESOLVED 2026-05-14 (PR #243).**
...
**Status: RESOLVED 2026-05-13 (PR #241).** `cli._purge_dir` now catches the full `OSError` hierarchy on the `f.unlink()` arm. `FileNotFoundError` still short-circuits silently (the vanished-path idiom from #109); any other `OSError` is logged as a WARNING naming the surviving file. Cascade trigger: when #122's 30s-timeout fires on Windows, the still-live daemon's `daemon.lock` handle is open, `Path.unlink` raises `PermissionError` (subclass of `OSError`), and pre-fix the user saw a Python traceback instead of a partial-cleanup report. Regression test in `test_install.py::test_purge_dir_continues_after_permission_error` monkeypatches `Path.unlink` to raise `PermissionError` selectively for `daemon.lock` and asserts (a) other files still get cleaned, (b) the locked file survives, (c) a WARNING surfaces the surviving file. Filed and resolved in the same PR. Surfaced 2026-05-13 alongside #122. **Codex P2 follow-up in the same PR:** the initial fix logged the failure but didn't feed it into `uninstall --purge`'s exit-2 gate — `_purge_dir`'s OSError arm dropped after warning, so a script gating on `$?` would conclude the purge had succeeded while `daemon.lock` (or any other locked state-dir file) remained on disk. Mirroring the `errors=`-list pattern already used by marker errors and `uninstall_shell_integration_if_supported`, `_purge_dir` now accepts an optional `errors: list[tuple[Path, str]]` and `_purge_local_state` threads it through; `uninstall` consolidates into a `state_errors` bucket, renders the "Could not fully purge state files (N errors)" stderr report mirroring the marker / shell-integration reports, and adds `state_errors` to the `if errors or shell_errors or state_errors: sys.exit(2)` gate. Two new regression tests pin the new shape: `test_purge_dir_records_errors_when_list_supplied` (unit-level — verifies `(path, message)` tuple accumulation) and `test_purge_exits_two_when_state_cleanup_fails` (end-to-end — `CliRunner`-driven `uninstall --purge` against a monkeypatched `Path.unlink` that locks `daemon.lock`; asserts exit 2 + stderr report). The original `test_purge_dir_continues_after_permission_error` exercises the `errors=None` default path so the backward-compat shape stays covered.

Touches: `src/dbxignore/cli.py:_purge_dir`, `_purge_local_state`, `uninstall` (~30 LOC across the three); `tests/test_install.py` (three tests total).
...
**Status: RESOLVED 2026-05-14 (PR #246).** Took fix candidate 1. `set_ignored` snapshots which detected attrs were already set (`_attr_present` helper, mirroring `is_ignored`'s per-attr test), then tracks the attrs *this call newly creates*; on any `setxattr` failure it clears only those newly-created attrs (per-attr `removexattr`, `symlink=True`) before re-raising the original exception, so a dual-attr path is left exactly as it was before the call — never half-marked, and never with a pre-existing marker on the inactive sync stack stripped off. A failure during the rollback clear is logged at WARNING and does not mask the original write exception. The docstring's old "partial state propagates" paragraph is rewritten to describe the rollback and to name *why* half-marked state is harmful (`is_ignored`'s first-hit-wins short-circuit hides it from the next sweep's retry). Three new tests in `tests/test_macos_xattr_unit.py`: `test_set_ignored_rolls_back_first_attr_on_second_write_failure` (both-mode, second `setxattr` raises `EDQUOT` after the first succeeds — asserts the newly-created first attr is cleared and `EDQUOT` propagates), `test_set_ignored_rollback_failure_does_not_mask_original_error` (rollback `removexattr` itself raises `EACCES` — asserts the original `EDQUOT` still propagates), and `test_set_ignored_rollback_preserves_preexisting_attr_in_both_mode` (the first attr pre-existed — asserts the rollback does *not* remove it; Codex follow-up on PR #246). No user-visible CLI surface change, so no manual-test-script extension needed.
...
**Status: RESOLVED 2026-05-14 (PR #249).** Closed by the DBXIGNORE_TEST_FAIL_* meta-fix — `STATE_PURGE` fail point + manual-test Phase 6 cases.
...
## 128. Manual-test Phase 6 doesn't exercise the new `launchctl bootout` confirmed-failure exit-2 path

`manual-test-macos.sh` Phase 6 doesn't exercise the new bootout confirmed-failure exit-2 path introduced by PR #242 (BACKLOG #119). The idempotent-success path (bootout returns "No such process" stderr after a manual `launchctl bootout` between install and uninstall) IS covered inline by the new 6c case. The remaining gap is the "service refused to die" path — bootout returns a non-zero rc with stderr that doesn't match `_NOT_LOADED_STDERR_PATTERNS` (e.g. `Boot-out failed: 5: Input/output error` from a degraded GUI session, or hypothetical "service is busy and won't unload" errors). Unit tests in `tests/test_macos_launchd.py::test_uninstall_agent_raises_on_bootout_nonzero_rc` pin the contract end-to-end at the function boundary, but the manual script can't observe the CLI's exit-2 surface for this path without forcing the underlying launchctl failure.

**Fix candidates:**

- (1) **Defer until a reproducible trigger emerges.** Forcing a non-not-loaded bootout failure reliably across macOS versions needs either root-level launchd state manipulation (unsafe to script for testers) or a known-broken environment (e.g. SSH-without-GUI-session-attached — but that's already an install-time failure, not a bootout-time failure). If a real-world failure mode surfaces during release testing, file the manual-test extension against that specific repro.
- (2) **Mock-injection via env-var override.** Add a `DBXIGNORE_TEST_FAIL_BOOTOUT=1` env var that `uninstall_agent` honors to simulate a non-zero-rc bootout. Cost: production-code coupling to a test path; bigger blast radius than the symptom warrants.

**Status: RESOLVED 2026-05-14 (PR #249).** Closed by the DBXIGNORE_TEST_FAIL_* meta-fix — `BOOTOUT` fail point + manual-test-macos.sh Phase 6 case.

**Urgency:** low. Unit tests pin the contract tightly. Same shape as #121 and #127 — "new exit-2 path, hard to force the underlying failure mode." Bundle with #121 / #127 if any of the three ever escalate. Surfaced 2026-05-13 during PR #242's work on resolving #119.
...
## 129. Manual-test Phase 6 doesn't exercise the new daemon-alive purge-refusal exit-2 path

Manual-test Phase 6 doesn't exercise the new daemon-alive guard's exit-2 path on `dbxignore uninstall --purge` introduced by PR #243 (BACKLOG #122). Unit tests in `tests/test_install.py::{test_purge_refuses_when_daemon_alive, test_purge_refuses_when_state_json_unreadable}` pin the contract end-to-end at the `CliRunner.invoke` surface. The happy-path negative-assertion guard ("no spurious daemon-alive guard fire on happy path") is added inline to all three Phase 6 scripts. The failure-path is deferred because forcing the underlying condition — a daemon that survives `uninstall_service`'s shutdown ceremony — requires a stuck filesystem operation or test-injection hook that doesn't reproduce on a healthy CI environment.

**Rule-of-four tripwire fired** (from BACKLOG #128's body): this is the fourth deferral matching the "new exit-2 path, hard to force the underlying failure mode" shape (#121, #127, #128, #129). Per #128's tripwire prescription, the next contributor with the bandwidth should revisit candidate (2) from #128's body — a `DBXIGNORE_TEST_FAIL_*` env-var injection mechanism that production code honors at strategic boundaries to simulate the failure modes that real platforms refuse to reproduce on demand. Implementing it would close all four deferrals at once and convert their unit-test-only coverage into end-to-end manual-test coverage.

**Fix candidates:**

- (1) **Defer until the rule-of-four meta-fix lands.** Status quo for any single new deferral. Cheap.
- (2) **Implement the meta-fix from #128.** Production code grows env-var-keyed hooks at four boundaries (one per existing deferral): `markers.is_ignored` for #121 ENOTSUP simulation; `_purge_local_state` for #127 OSError simulation; `launchctl bootout` rc for #128; daemon-alive check for #129. Each hook gated on its own `DBXIGNORE_TEST_FAIL_*` env var, refusing to fire unless the env var is set. Costs: small production-code surface; coupling to a test-only mechanism. Returns: end-to-end coverage of four current deferrals + a forward-compatible mechanism for the next one.

**Status: RESOLVED 2026-05-14 (PR #249).** Closed by the DBXIGNORE_TEST_FAIL_* meta-fix — `DAEMON_ALIVE` fail point + manual-test Phase 6 cases. The rule-of-four meta-fix (#128's tripwire) landed: a shared `_testing.py` injection mechanism closing all four deferrals at once.

**Urgency:** low (for the individual deferral); medium (for the meta-fix — fourth occurrence of the same pattern is the signal the tripwire was designed for). Unit tests pin the contract; happy-path guard catches the most likely regression class. Surfaced 2026-05-14 during PR #243's work on resolving #122.

Touches: `scripts/manual-test-{windows.ps1,ubuntu-vps.sh,macos.sh}` Phase 6 (failure-path simulation); `src/dbxignore/cli.py` (env-var hook at the daemon-alive guard boundary if candidate 2 is taken — and parallel hooks for #121 / #127 / #128's respective boundaries).
...
## 137. `install` opens an empty console window when dbxignore is installed via uv

**Status: RESOLVED 2026-05-14 (PR #252).** `install._common.detect_invocation` selected the daemon's Task Scheduler launcher by checking whether `pythonw.exe` *exists* next to `sys.executable`, on the assumption that `pythonw.exe` is the windowless GUI-subsystem interpreter. That assumption is false for uv-created virtualenvs: uv ships `pythonw.exe` as a byte-identical copy of the console-subsystem `python.exe` trampoline (both PE Subsystem 3). Task Scheduler's logon action then launched a console-subsystem process under `InteractiveToken`, so Windows allocated a visible console that stayed open for the daemon's whole lifetime (empty, because the daemon only logs to a file on Windows). This hit every uv-based install — `uv tool install dbxignore`, `uv run` from a dev checkout — i.e. the documented primary install path. Fix: declare a `dbxignorew` `[project.gui-scripts]` entry point so pip/uv generate a real GUI-subsystem trampoline (`dbxignorew.exe`, PE Subsystem 2 — verified end-to-end), the non-frozen analogue of the bundled frozen `dbxignorew.exe`. `detect_invocation`'s non-frozen Windows branch now prefers the `dbxignorew.exe` sibling of `sys.executable`, then `shutil.which("dbxignorew")` (the `uv tool install` bin-dir layout), then `pythonw.exe` **only after** `_is_gui_subsystem()` confirms its PE Subsystem field is Windows GUI, then `python.exe` with a WARNING. `_is_gui_subsystem` reads the MZ → `e_lfanew` → `PE\0\0` → optional-header Subsystem field (offset 68, identical for PE32/PE32+); any read/parse failure returns False so the caller falls back to a launcher it can reason about. Four new tests in `tests/test_install_common.py` (sibling preference, PATH preference, genuine-GUI-pythonw acceptance, console-subsystem-pythonw rejection + the no-GUI-launcher fallback) with a minimal fake-PE writer helper; the old `test_detect_invocation_returns_pythonw_on_windows` (which encoded the name-only assumption) was replaced. Manual-test `manual-test-windows.ps1` Phase 5-pre reworked: it renames `pythonw.exe` away and now asserts install stays windowless via `dbxignorew.exe` with no fallback warning (pre-fix that rename forced the console-flash warning), and the Phase 5 task-XML check is tightened from "dbxignorew.exe or pythonw.exe" to "dbxignorew.exe" specifically. Filed and resolved in the same PR; surfaced 2026-05-14 from a user report of `uv run dbxignore install` opening an empty terminal window titled with the venv `pythonw.exe` path.

Touches: `pyproject.toml` (`[project.gui-scripts]`); `src/dbxignore/install/_common.py` (`_is_gui_subsystem` + non-frozen Windows branch of `detect_invocation`); `tests/test_install_common.py` (fake-PE helper + 5 tests); `scripts/manual-test-windows.ps1` (Phase 5-pre + Phase 5 task-XML check).

---

## 138. `detect_invocation` trusts a `dbxignorew.exe` whose companion `pythonw.exe` is console-subsystem (follow-up to #137)

**Status: RESOLVED 2026-05-14 (PR <THIS_PR>).** #137's fix fixed every real user-facing install path (`uv tool install`, `pip install`, the frozen `.exe`), but left a residue on the `uv run dbxignore install` *developer* path. Root cause one layer deeper than #137 addressed: a non-frozen `dbxignorew.exe` is a pip/uv GUI-script *trampoline* — it is GUI-subsystem itself but re-execs the sibling `pythonw.exe` to do the real work. It is therefore only genuinely windowless when that `pythonw.exe` is also GUI-subsystem. `uv tool install` venvs ship a real GUI-subsystem `pythonw.exe` (verified: PE Subsystem 2), so their `dbxignorew.exe` is windowless — #137's preference for it is correct there. But uv *project* venvs (`uv sync` / `uv run`) ship a 47 KB **console-subsystem** `pythonw.exe` trampoline, so their `dbxignorew.exe` still allocates a console window despite being GUI-subsystem. #137's `detect_invocation` step 1 returned the `dbxignorew.exe` sibling unconditionally — a windowed launcher dressed up as a windowless one. Process-tree evidence: `dbxignorew.exe` (GUI) → `.venv\Scripts\pythonw.exe` (console) → `conhost.exe` (the window) + base `python.exe`. Fix (the honest-refinement option): new `_dbxignorew_launches_windowless(dbxignorew)` helper checks the sibling `pythonw.exe`'s PE Subsystem field; `detect_invocation` step 1 is gated on it, so a console-delegating `dbxignorew.exe` sibling falls through to the existing fallback chain and ultimately the WARNING. The WARNING text was rewritten to name the `uv sync` / `uv run` console-subsystem-trampoline cause and advise `uv tool install` / `pip install`. No attempt to make `uv run install` itself windowless — uv project venvs have no windowless launcher at all, and the heavy options (hide-console-at-startup, `conhost --headless`, `.vbs` shim) aren't warranted for a developer-only path that no end user uses. New unit test `test_detect_invocation_windows_rejects_dbxignorew_sibling_with_console_pythonw`; two existing tests' WARNING-substring assertions updated to the new text. Manual-test `manual-test-windows.ps1` Phase 5-pre reworked from the #137 rename-based test (now invalid — a `dbxignorew.exe` with a missing/console companion `pythonw.exe` is correctly distrusted) to a static PE-header check that the `uv tool install` venv's `pythonw.exe` is GUI-subsystem; the Phase 7 `.bak-5pre` restore backstop was removed with it (no more file manipulation). Filed and resolved in the same PR; surfaced 2026-05-14 when the #137 reporter confirmed the window still appeared under `uv run`.

Touches: `src/dbxignore/install/_common.py` (`_dbxignorew_launches_windowless` + step-1 gate + WARNING text + docstrings); `tests/test_install_common.py` (one new test + two assertion updates); `scripts/manual-test-windows.ps1` (Phase 5-pre rewrite + Phase 7 backstop removal).
...
- **#138** (2026-05-14, PR <THIS_PR>) — follow-up to #137: `detect_invocation` no longer trusts a `dbxignorew.exe` sibling whose companion `pythonw.exe` is console-subsystem. A non-frozen `dbxignorew.exe` is a GUI-script trampoline that re-execs the sibling `pythonw.exe`, so it is windowless only when that `pythonw.exe` is GUI-subsystem — true for `uv tool install` / `pip` venvs, false for uv project venvs (`uv sync` / `uv run`). New `_dbxignorew_launches_windowless` helper gates step 1; the fallback WARNING now names the cause and advises `uv tool install` / `pip install`. `uv run dbxignore install` (developer-only path) intentionally still shows a console window — uv project venvs ship no windowless launcher — but now warns about it. Filed and resolved in the same PR.
- **#137** (2026-05-14, PR #252) — `install` no longer opens an empty console window for the daemon's lifetime on uv-based Windows installs. `detect_invocation` trusted `pythonw.exe` by name as the windowless launcher, but a uv-built virtualenv's `pythonw.exe` is a console-subsystem copy of `python.exe`, so Task Scheduler's logon action allocated a visible console. Fix: a `dbxignorew` `[project.gui-scripts]` entry point (pip/uv generate a real GUI-subsystem trampoline — the non-frozen analogue of the bundled `dbxignorew.exe`), preferred by `detect_invocation` over `pythonw.exe`, which is now used only after `_is_gui_subsystem()` verifies its PE Subsystem field. Filed and resolved in the same PR from a user report.
- **#121** (2026-05-14, PR #249) — `clear` / `list` `scan_errors` exit-2 path now exercised end-to-end by manual-test case 4u (bash helper) / 4v (Windows). Closed by the `DBXIGNORE_TEST_FAIL_*` meta-fix: the new `_testing.raise_if_fail_point("MARKER_READ")` hook fires before every `markers.is_ignored` read in `cli._walk_marked_paths` (the `target` read and both walk-loop reads), so an injected `OSError` lands in the existing `except OSError` arm and drives `scan_errors`. Part of the rule-of-four meta-fix (see #129).
- **#127** (2026-05-14, PR #249) — `uninstall --purge` `state_errors` exit-2 path now exercised end-to-end by new Phase 6 cases in all three manual-test scripts. Closed by the `DBXIGNORE_TEST_FAIL_*` meta-fix: the `STATE_PURGE` hook in `cli._purge_dir`'s unlink loop raises `OSError` into the existing `except OSError` arm. The Phase 6 case re-installs first so the daemon has written state files for the unlink loop to fail against. Part of the rule-of-four meta-fix (see #129).
- **#128** (2026-05-14, PR #249) — `uninstall` `launchctl bootout` confirmed-failure exit-2 path now exercised end-to-end by a new `manual-test-macos.sh` Phase 6 case. Closed by the `DBXIGNORE_TEST_FAIL_*` meta-fix: the `BOOTOUT` hook in `macos_launchd.uninstall_agent` substitutes a non-zero-rc `CompletedProcess` whose stderr `_is_service_not_loaded` does not match, so the existing `raise RuntimeError` arm fires. This item's body planted the rule-of-four tripwire; #129 fired it and the meta-fix landed here. Part of the rule-of-four meta-fix (see #129).
- **#129** (2026-05-14, PR #249) — `uninstall --purge` daemon-alive purge-refusal exit-2 path now exercised end-to-end by new Phase 6 cases in all three manual-test scripts. **This is the rule-of-four meta-fix.** New module `src/dbxignore/_testing.py` provides `raise_if_fail_point` / `fail_point_active`, both env-var-keyed and inert by default, logging a WARNING when armed. Four one-line hook sites (`MARKER_READ`, `STATE_PURGE`, `BOOTOUT`, `DAEMON_ALIVE`) trigger already-unit-tested exit-2 paths the manual-test shell scripts otherwise could not reach. The `DAEMON_ALIVE` hook fires `cli.uninstall`'s `--purge` daemon-alive gate directly. Closes #121, #127, #128, #129 together — converting four unit-test-only contracts into end-to-end manual-test coverage and leaving a forward-compatible mechanism (a new fail point is a one-line hook + a docstring entry) documented in the AGENTS.md Gotchas section.
- **#106** (2026-05-14, PR #247) — `format_literal_rule` now uses the `_is_real_dir(target)` helper instead of the inline `target.is_dir() and not target.is_symlink()` two-call form, so all three call sites (`cache.match`, `cache.explain`, `format_literal_rule`) share the single-`lstat()` helper. The six MagicMock-based tests in `test_rules_format_literal_rule.py` were migrated: the three dir-case tests stub `target.lstat.return_value.st_mode = stat.S_IFDIR` instead of `is_dir`/`is_symlink`; the three whitespace-rejection tests raise `ValueError` before the dir check, so their `is_dir`/`is_symlink` stubs were dead setup and were dropped. `_is_real_dir`'s docstring drops the "format_literal_rule still uses the two-call form" parenthetical. Pure code-cleanliness — `format_literal_rule` is cold-path so the syscall saving is irrelevant.
- **#107** (2026-05-14, PR #247) — `symlink_capable` runtime-probe fixture moved from `test_cli_symlink_path_args.py` to `tests/conftest.py` (next to the sibling `require_case_sensitive_fs` probe; `conftest.py` gained `import os`). Five tests dropped their `@pytest.mark.skipif(sys.platform == "win32")` decorator and added `symlink_capable: None` as a fixture parameter: three in `test_cli_ignore.py` and two in `test_rules_basic.py` — the call sites whose skip reason was specifically "Windows symlink creation requires admin privileges". The item's "~10 tests" estimate was high; the other `skipif(win32)` sites in those files are filename-restriction or platform-behavioral skips, not symlink-capability gates, and stay. Verified on a Windows host with Developer Mode: all five now run and pass instead of skipping (portable suite 702→707 passed, 38→33 skipped on Windows) — the runtime probe means CI's `windows-latest` runner gets real symlink coverage where it previously got none.
- **#125** (2026-05-14, PR #246) — `set_ignored` (`_backends/macos_xattr.py`) now leaves a dual-attr path exactly as it was before the call when a write partially fails. Pre-fix, in dual-attr mode a first-write success followed by a second-write failure left the first xattr set; `is_ignored`'s first-hit-wins short-circuit then reported the path as fully ignored, so the next sweep never retried the missing second write — leaving the path marked for one sync stack but not the other. Fix candidate 1: snapshot which attrs already existed (`_attr_present` helper), track the attrs this call newly creates, and on any `setxattr` failure clear only those newly-created ones (`removexattr`, `symlink=True`) before re-raising the original exception. A rollback-clear failure is logged at WARNING without masking the original write exception. Single-attr mode is unaffected (nothing newly created when the lone write fails, so the rollback loop is a no-op). Three new tests in `tests/test_macos_xattr_unit.py` pin the rollback, the rollback-failure-doesn't-mask-original behavior, and the preserve-pre-existing-attr behavior (the last added in response to a Codex review comment on PR #246 — an unconditional rollback would have stripped a marker that pre-existed on the inactive sync stack). No user-visible CLI surface change. Surfaced 2026-05-13 in the external-review pass that produced #122 / #123 / #124 / #126.
- **#122** (2026-05-14, PR #243) — `cli.uninstall`'s `if purge:` block now opens with a daemon-alive fail-closed gate that fires after `uninstall_service` returns but before the destructive purge body (marker clearing + state-dir wipe). Two-arm structure mirrors `cli.clear`'s precedent at `cli.py:992-1014` — Arm A refuses when `state.json` exists on disk but `state.read()` returns None (daemon liveness unknown — locked file, malformed JSON, permission-denied); Arm B refuses when `state.read()` returns a valid State AND `state.daemon_is_running(s)` reports True (the canonical Windows 30s-`schtasks /End`-timeout case). Both arms exit 2 with stderr documenting the manual recovery path (kill daemon via `taskkill /F /PID <pid>` on Windows, run `dbxignore clear --force` for markers, manually `rm -r` the state directory). No `--force` escape hatch: the failure mode being fixed IS silent tug-of-war, and a flag that bypasses the gate would re-enable exactly the bug we're closing. Three new tests in `tests/test_install.py`: `test_purge_refuses_when_daemon_alive` pins Arm B against a monkeypatched `state.is_daemon_alive=True` + marker-survival + state.json-survival assertions; `test_purge_refuses_when_state_json_unreadable` pins Arm A against a malformed-JSON state file; `test_purge_proceeds_when_daemon_dead` is the regression guard that prevents the gate from over-firing on the common clean-uninstall case. Happy-path negative-assertion guard added to all three manual-test scripts' Phase 6 mirroring the PR #204 / PR #241 pattern; failure-path manual-test coverage filed as BACKLOG #129 — same shape as #121, #127, #128, triggering the rule-of-four tripwire from #128's body (four-of-a-kind now). Took fix candidate (2) from the body — the lighter re-poll-`daemon_is_running` approach using the canonical public helper — without holding `daemon._acquire_singleton_lock` because that daemon-internal helper isn't exported and the state-based boundary check is sufficient. Platform-agnostic: Linux's `systemctl --user disable --now` and macOS's `launchctl bootout` (post-PR #242) are synchronous, so by the time the gate evaluates the daemon is guaranteed dead and the gate returns False; the gate only meaningfully fires on Windows. **Codex P2 follow-up in the same PR**: the initial Arm A shape refused unconditionally when `state.json` was unreadable, blocking `--purge`'s documented recovery flow for the orphaned-corrupt-state-file case (Linux/macOS daemons are guaranteed dead by the time the gate fires; Windows daemons that stopped normally but left a bad state.json have the same shape). Followup adds `state.is_any_daemon_running()` — a no-PID liveness probe that briefly acquires-then-releases the daemon-singleton lock at `daemon.lock` (the daemon holds an exclusive `fcntl.flock` / `msvcrt.locking` on this file for its entire lifetime per `daemon._acquire_singleton_lock`). Arm A now refuses only when state.json is unreadable AND the lock-probe reports a contender; when no daemon process is detected, `--purge` proceeds and cleans up the corrupt state.json. New test `test_purge_proceeds_when_state_unreadable_and_no_daemon` pins the recovery path; the existing `test_purge_refuses_when_state_json_unreadable` keeps the strict-refusal path covered via `monkeypatch.setattr(state, "is_any_daemon_running", lambda: True)`. Four new helper tests in `tests/test_state.py` (no lock file → False, empty lock file → False, acquirable lock → False, held lock → True via `daemon._acquire_singleton_lock`) cover the helper end-to-end. **Codex P2 follow-up #2 in the same PR**: the first Codex P2 fix preserved the two-arm shape, with Arm A refusing only when `state.json` was unreadable AND a daemon was detected via the lock probe. Codex correctly flagged that this missed the *missing* state.json case — when `state.json` is absent entirely (prior partial `--purge` removed it but couldn't delete the held `daemon.lock`; user manually `rm`'d it; etc.), `state.read()` returns None and `state.default_path().exists()` is False, so neither arm fires and a live daemon races the purge body unprotected. Second-iteration fix collapses the two arms into one unified gate: always probe `is_any_daemon_running()` (lock-based, authoritative), plus the standard PID-anchored check when state.json is readable; either positive signal fires the gate. Error message uses one of two PID-clause variants — `(pid=X)` when the PID-anchored check confirms the live process, or `(daemon.lock is held; PID unknown ...)` when only the lock-probe signal is available. New test `test_purge_refuses_when_state_json_missing_and_daemon_alive` pins the missing-state coverage; the four prior gate tests pass unchanged against the unified shape.

#### 2026-05-13

- **#119** (2026-05-13, PR #242) — `uninstall_agent`'s `launchctl bootout` call now captures rc + stderr and distinguishes "service was already unloaded" (idempotent success — proceed to plist removal) from "service refused to die" (real failure — raise `RuntimeError`, preserve plist on disk for investigation). Stderr-tolerant matching against a new module-level `_NOT_LOADED_STDERR_PATTERNS` tuple (`"no such process"`, `"could not find service"`, `"could not find specified service"`, `"not loaded"`, case-insensitive via `_is_service_not_loaded`) keeps `dbxignore uninstall` idempotent for the repeat-call and "user manually ran `launchctl bootout`" cases while ensuring a confirmed bootout failure surfaces as exit-2 at the CLI surface — mirroring the eventual-consistency contract Linux/Windows uphold through their respective service-managers. Builds on PR #240's partial fix (the OSError invocation arm); the rc-swallow arm is the second half of the same finding. Two new tests in `tests/test_macos_launchd.py`: `test_uninstall_agent_raises_on_bootout_nonzero_rc` pins the strict-fail-loud path against a `Boot-out failed: 5: Input/output error` simulation and asserts the plist survives; `test_uninstall_agent_tolerates_not_loaded_stderr` pins the idempotent path against a `Boot-out failed: 3: No such process` simulation and asserts the plist is removed. Took a hybrid of the body's fix candidates (1) and (3) — log on idempotent failure, raise on confirmed failure — without (2)'s `launchctl print` poll because stderr alone suffices to distinguish the two cases.

- **#123** + **#124** (2026-05-13, PR #241) — External-review followups, filed and resolved in the same PR. **#123**: `daemon.run()`'s hourly while loop now wraps `_sweep_once` in `try/except Exception: logger.exception("hourly sweep tick failed; continuing")`. Pre-fix an unexpected non-`OSError` exception (`AttributeError`, `KeyError`, pathspec edge case) would propagate out of the loop and force a service-manager restart every hour until the underlying bug was found. The `_initial_sweep_worker` carries the symmetric wrap (item #91); this closes the asymmetry. **#124**: `cli._purge_dir` widened its `f.unlink()` suppress from `FileNotFoundError` to the full `OSError` hierarchy, with WARNING-and-continue on non-vanished-path errors. Cascade trigger: when #122's 30s-timeout fires, the still-live daemon holds `daemon.lock` open; pre-fix the user saw a Python traceback instead of a partial-cleanup report. **Codex P2 follow-up in the same PR**: the initial #124 fix only logged the OSError — the failure never reached `uninstall --purge`'s exit-2 gate, so a script gating on `$?` could conclude the purge had succeeded while a dbxignore artifact remained on disk. Followup adds an `errors=` list kwarg to `_purge_dir` and `_purge_local_state` mirroring the existing marker / shell-integration pattern, plus a new `state_errors` bucket in `uninstall` with a matching stderr report and exit-2 gate addition. Two additional regression tests (`test_purge_dir_records_errors_when_list_supplied`, `test_purge_exits_two_when_state_cleanup_fails`) pin the new shape; the original `test_purge_dir_continues_after_permission_error` continues to cover the `errors=None` default path. Regression tests for the load-bearing pieces: `test_daemon_initial_sweep.py::test_hourly_tick_recovers_from_sweep_exception` (#123 counted-call sweep) and `test_install.py::test_purge_dir_continues_after_permission_error` + the two follow-up tests (#124 + the Codex P2 follow-up).
...
- **#30** (2026-05-13, PR #238) — Single `dbxignore` command everywhere. `[project.scripts].dbxignored` dropped from pyproject.toml; PyInstaller Windows + macOS specs collapsed to one EXE / Mach-O each; new `src/dbxignore/_windows_console.py` module (~110 LOC stdlib ctypes) performs `AttachConsole(ATTACH_PARENT_PROCESS)` early in startup via `__main__.main_entry()` (the deferred-cli-import dance ensures rich-click sees the redirected stdio). Three-context behavior on Windows: terminal launch (per-stream stdio preservation), Task Scheduler logon (silent), Explorer double-click (`user32.MessageBoxW`). Installers (Linux systemd, macOS launchd, Windows Task Scheduler) updated to invoke `dbxignore daemon`. `install/_common.py:detect_invocation` simplified; `state.is_daemon_alive` guard tuple replaced from `("python", "dbxignored")` with `("python", "dbxignore")`; `.github/workflows/release.yml` pruned. Known limitation: Windows shells don't wait for GUI-subsystem foreground invocations (documented per-shell sync workarounds in README). Twelve new tests in `tests/test_windows_console.py` cover the orchestrator decision branches plus the `_is_stream_connected` predicate plus per-stream `_redirect_stdio_to_attached_console` mixed-case behavior. Migration via `dbxignore uninstall` before upgrading + `dbxignore install` after. Took fix candidate (1) from the body — stdlib ctypes, no pywin32. Resolves a pre-1.0 SemVer Breaking change targeting v0.6.0.

#### 2026-05-12

- **#114** (2026-05-12, PR #234) — CLI default log level lowered from `INFO` to `WARNING`. `--verbose` / `-v` shifted from `is_flag=True` to click's counted form (`count=True`): `-v` selects `INFO` (was `DEBUG`), and a new `-vv` form selects `DEBUG`. Pre-fix, every CLI invocation emitted `logger.info` chatter from install backends (`Wrote systemd user unit to …`, `Bootstrapped … into gui/…`) alongside the intentional `click.echo` summary lines; after, only the `click.echo` summaries surface by default. Took fix candidate (1) — the preferred group-level flag with the spec's WARNING default — extended with the user-confirmed `-vv` form to preserve DEBUG access for power users without losing the spec's primary goal. New helper `cli._verbosity_to_level(verbose: int) -> int`; `daemon_main` gets the same change for symmetry (its `basicConfig` is briefly active before `daemon.run()`'s `_configured_logging` takes over via `DBXIGNORE_LOG_LEVEL`). Five new tests in `tests/test_cli_entrypoints.py` cover the helper's four paths (`0 → WARNING`, `1 → INFO`, `2 → DEBUG`, `3+ → DEBUG` clamp) plus a `-vv`-is-reachable end-to-end. No `logger.info` → `click.echo` audit conversions needed — the existing CLI-level `click.echo` summaries already cover user-facing output. CHANGELOG entry under `[Unreleased]` > Changed, marked **Breaking** since `--verbose` semantics shift and default level drops.
...
- **#101** (2026-05-11, PR #207; tracker-hygiene backfill close on later date) — `rules.append_rule()` and `rules.remove_rule()` now use `tempfile.mkstemp(dir=rule_file.parent, prefix=..., suffix=...)` so each writer picks a unique sibling temp file; two concurrent CLI invocations, an editor's atomic-save backup, or a user-created `.dropboxignore.tmp` cannot collide on the path. The same PR also captures the existing file's POSIX mode (or `0o666 & ~umask` for new files) and `os.chmod`s the temp before the replace, so shared-workflow setups keep their group-readable mode (`mkstemp` creates at `0o600` by default; without the chmod, `os.replace` would silently relock group-readable `.dropboxignore` files to user-only after the first `ignore`/`unignore` invocation). The lost-update window (two readers, two writers, replace race) remains; an advisory lock is deferred until a concrete failure shows. CHANGELOG entry for v0.5.0 said "Resolves BACKLOG #101" but the BACKLOG item itself never got the Status marker — pure bookkeeping miss, same shape as #103. Note that PR #207 also resolved #103 (CI canonical pytest invocation) — single PR closing two items.

- **#102** (2026-05-11, PR #206; tracker-hygiene backfill close on later date) — The cache-invalidation gate in `RuleCache._load_if_changed()` switched from `(mtime_ns, size)` stat values to a blake2b-128 hash of the file's bytes. Same heuristic-vs-correctness trade as git's `core.checkStat` and rsync's default mode: previously missed when an editor preserved mtime AND the new content was the same size, caught cleanly by the hash. The expensive part of the load (pathspec compile) is still skipped when content is unchanged, so the optimization that the `(mtime, size)` shortcut was originally meant to provide is preserved. CHANGELOG entry for v0.5.0 said "Resolves BACKLOG #102" but the BACKLOG item itself never got the Status marker — pure bookkeeping miss, same shape as #103.

- **#105** (2026-05-11, PR #205; tracker-hygiene backfill close on later date) — All path-taking verbs now refuse paths where `..` follows a symlinked component. The validator's sticky-flag scan walks the un-normalized absolute path's segments, tracks `seen_symlink` via `os.path.islink(prefix)`, and exits 2 on the first `..` that follows a symlinked prefix. A `..` *before* any symlink remains accepted (it cancels a regular segment with no FS-divergence). The guard fires only when the lexical interpretation will be used — out-of-Dropbox alias paths that fall through to the existing `path.resolve()` fallback are unaffected (the resolve fallback is filesystem-true and unambiguous). Took fix candidate (1) — the preferred loud refusal of ambiguous shapes, matching the symlinked-ancestor guard's spirit. CHANGELOG entry for v0.5.0 said "Resolves BACKLOG #105" but the BACKLOG item itself never got the Status marker — pure bookkeeping miss, same shape as #103.

- **#98** (2026-05-11, PR #204; tracker-hygiene backfill close on later date) — `uninstall --purge` accumulates per-path marker errors as `(path, operation, message)` tuples (operation tagged `read` during `markers.is_ignored` or `clear` during `markers.clear_ignored`), prints up to 10 on stderr in the form `Could not fully clear markers (N errors):` + `  <op> failed on <path>: <message>` with an `... and X more.` tail when applicable, and exits 2 on any failure. The prior `except OSError: pass` shape silently swallowed permission errors, unsupported xattr/ADS operations, transient EIO, and symlink xattr edges; users saw `Cleared N` regardless of actual outcome. Local-state cleanup still runs after a partial marker failure — only the exit code and stderr report are new. `FileNotFoundError` from a path that vanished between `os.walk` and our `markers.is_ignored`/`clear_ignored` call is treated as a non-error (mirrors `reconcile._reconcile_path`'s read-arm vanished-path treatment). CHANGELOG entry for v0.5.0 said "Resolves BACKLOG #98" but the BACKLOG item itself never got the Status marker — pure bookkeeping miss, same shape as #103.

- **#97** (2026-05-11, PR #203; tracker-hygiene backfill close on later date) — `state._read_at()` broadened to catch generic `OSError` (permission, lock, cloud-placeholder, transient unavailability) on top of the existing `JSONDecodeError`/`KeyError`/`TypeError`/`ValueError` arm; logs `WARNING` and returns `None` so `dbxignore status`, `clear`'s daemon-alive guard, `daemon.run`'s legacy-state migration path, and Windows uninstall's daemon-exit wait all degrade gracefully rather than crash. Dropped the pre-`exists()` LBYL check in favor of catching `FileNotFoundError` from `read_text()` itself (closes the exists/read TOCTOU window). `cli.clear`'s daemon-alive guard additionally fails closed (exit 2 with `error: state.json at <path> is present but unreadable; daemon liveness is unknown.`) rather than treating "unknown" as "no daemon" and racing the live daemon's marker re-apply; `--force` overrides. CHANGELOG entry for v0.5.0 said "Resolves BACKLOG #97" but the BACKLOG item itself never got the Status marker — pure bookkeeping miss, same shape as #103.
...
- **#92** in PR #185 — comprehensive case-insensitive rule-filename predicate. Took fix candidate (1). Two new helpers in `rules.py`: `is_ignore_filename(name: str) -> bool` (case-insensitive `.dropboxignore` predicate) and `_canonical_cache_key(path: Path) -> Path` (lowercase-basename cache-key normalization). Used at all the case-sensitivity sites flagged by Codex P2 #5 on PR #184: `match` and `explain` (predicate), `reload_file` and `remove_file` and `_load_file` and `_load_if_changed` (cache-key normalization), `daemon._classify` and `daemon._moved_dest_under_root` and `_dispatch`'s `src_is_rules` (predicate), `reconcile._reconcile_path` (predicate, for the rule-file-marked-ignored warning), `cli.uninstall --purge` arm (predicate, for the rule-file-marked-ignored auto-clear warning during the purge walk). End state: `.DropboxIgnore` and `.dropboxignore` are equivalent across discovery, watchdog, reconcile, and walk paths regardless of filesystem case sensitivity. Eight new tests cover the predicate (`test_is_ignore_filename_predicate`), `match`/`explain` short-circuit (two tests), `reload_file`/`remove_file` canonical-key normalization (two tests), `_reconcile_path` warning fires for mixed-case (one test), and `daemon._classify`/`_moved_dest_under_root` recognize mixed-case events (two tests). The `cli.uninstall --purge` use site reuses an existing untested branch (rule-file marker auto-clear is a project invariant restoration that's verified end-to-end by the existing manual-test scripts rather than a unit test).
...
- **#89** in PR #175 — slow-sweep marker hook for manual-test determinism. Took fix candidate (1) — daemon-side hook + per-script seed/cleanup — but with a framing correction: marker file at `state.user_state_dir() / "_test_slow_sweep"` rather than the prescribed `DBXIGNORE_TEST_SLOW_SWEEP_S` env var. The env-var path was clean for Linux/macOS (1-line `_FORWARDED_ENV_VARS` extension each) but Windows Task Scheduler XML has no per-task env-var forwarding, and even on Linux/macOS the existing forwarding tuple is scoped to `DBXIGNORE_ROOT` (other `DBXIGNORE_*` tuning vars are set via unit/plist drop-ins) — adding a TEST var would have been the only env var the install layer auto-forwards beyond config. The marker file works uniformly across all three platforms with zero install-layer surface change, and `_purge_local_state` cleans it up so a stale marker can't silently pad future installs. `daemon._slow_sweep_pad_seconds()` validates the value (mirrors `_timeouts_from_env` shape — WARNING when honored, WARNING-and-ignore on non-numeric/negative); call site at top of `_initial_sweep_worker` uses `stop_event.wait(pad_s)` so daemon shutdown stays prompt mid-pad. All three manual-test scripts seed `15` before `dbxignore install` and remove the marker at end of phase 5; the Linux script's existing `trap cleanup EXIT` arm catches phase-5 aborts, while the macOS/Windows scripts ship a `phase_cleanup` arm (happy-path-only — those scripts have no trap-equivalent today, so a mid-phase-5 abort can leave a stale marker the next `uninstall --purge` cleans up).
...
- **#88** in PR #172 — partial test-level cleanup + restored regression pin. The `Wait-Process -Id` workarounds + `$purgePid` capture (PR #169 commits `5c3bc5f`, `fde50c5`) are gone — `dbxignore uninstall` is now synchronous, so those compensations for the asynchronous-uninstall bug are no longer needed. **Codex P2 catch in the same PR (commit `17defe1`):** the cleanup over-shot. The daemon_pid-advance polling block is separate from the Wait-Process workaround and remains load-bearing even with synchronous uninstall — `state.json` is retained by plain uninstall, so a Test-Path-only check passes on the stale file immediately and `--purge` reads the dead pre-uninstall pid; `uninstall_task`'s synchronous wait then targets a long-dead pid and `/Delete` reverts to fire-and-forget against the live re-installed daemon (exactly the race #87 closes). Restored the daemon_pid-advance poll AND landed fix candidate (1) at the same time: explicit `Write-Pass`/`Write-Fail` so a future regression in the polling logic surfaces as a loud failure rather than the silent miss the item warned about. Variable names differ from the item's framing (`$oldPid` / `$reinstallPid` instead of `$uninstallPid` / `$purgePid`) because the workaround-supporting half of the original PID-dance was correctly retired. The "self-limiting lifespan" prediction was half right: the Wait-Process layer retired, but the polling-for-state-advance layer is structurally tied to plain-uninstall's state.json-retention contract, not to the asynchronous-uninstall bug.

#### 2026-05-08

- **#84** in PR #159 (Dependabot, backfilled in PR #178) — self-resolved. Dependabot's `ci(deps): bump actions/checkout from 4.3.1 to 6.0.2` (merged 2026-05-08T12:55:59Z, 38 minutes after #84 was filed in PR #156) bumped both lagging files (`claude.yml`, `claude-code-review.yml`) directly past v5 to v6.0.2. All six workflow files now pin `de0fac2e...` tagged `# v6.0.2`. The setup-uv v6/v7 arm of #84 went moot earlier when PR #163 retired `codex-followup.yml`. Closure mechanism matches the body's "may eventually self-resolve" prediction. The 24-hour bookkeeping gap was discovered while prepping a v4→v5 bump that turned out to be a no-op; logging the closure here keeps the at-a-glance Open list honest.

- **#85** in PR #163 — workflow retired. The codex-followup auto-fix workflow had a 3-commit budget per PR; PR #162's six-Codex-finding daemon refactor exhausted it, after which the bot's escalation replies were duplicative of the maintainer's own session-driven fixes. Rather than fix the workflows-write permission gap (which had real security trade-offs anyway), the workflow file was deleted entirely. Manual `@codex review` summon still works for Codex review pass; reply path is now maintainer-only. File recoverable via `git show <pre-PR-163>:.github/workflows/codex-followup.yml` if the auto-fix-on-small-PRs value is ever missed.

- **#53 candidate 1** in PR #162 — partial resolution; candidates 2 + 3 remain open (see item body). `daemon.run()` reordered so the singleton lock + observer + first `state.json` write happen before the initial sweep, which now runs on a `_initial_sweep_worker` thread. New `state=starting` token surfaces in `dbxignore status --summary` (and the human path distinguishes starting vs. running) — pre-1.0 breaking-ish addition for callers branching exhaustively on `state == "running"`, documented in CHANGELOG. Cooperative cancellation: `reconcile_subtree` and `_sweep_once` thread a `stop_event` so SIGTERM during the initial sweep returns within ~1s rather than waiting for the full traversal. Initial sweep wall-clock unchanged (~50s on 27k dirs); what changed is *when* readiness signals fire relative to the sweep, which removes the user-visible systemd-startup pause. Backfill close — body update missed at merge time, same shape as #42 (PR #124), #83 (PR #151), #84 (PR #178). Six Codex P2 findings on the same PR were addressed in the same merge train (commits `d3fed37` cancellation between `dirnames` entries, `03a8695` hold singleton lock until worker exits, `7196636` skip state write on cancelled sweep, `4e8f1e4` skip periodic sweep while initial worker alive, `98b26cb` defer `cache.load_root` to worker thread, `97d90de` cancellable rule scan in `RuleCache.load_root`); a seventh deferred-as-architectural finding was filed as item #86.

- **#74** in PR #156 — pinned all 22 GitHub Action references across the six `.github/workflows/*.yml` files to 40-char commit SHAs with trailing `# v<X>`-style comments naming the resolved tag. New `.github/dependabot.yml` with `package-ecosystem: github-actions`, weekly schedule, Conventional Commits prefix `ci(deps)` to satisfy `cchk.toml`, and an `actions-minor-patch` group bundling minor+patch bumps. Took fix candidate (option 1) — sweep all actions, not just release-critical. Two version-skew inconsistencies (`actions/checkout` v4 vs v5, `astral-sh/setup-uv` v6 vs v7) filed as item #84 for a separate version-modernization PR rather than bundled here, per CLAUDE.md's revertability-axis split rule. CLAUDE.md's `## Release` section gained a new bullet documenting the SHA-pin convention so future workflow additions don't degrade the trust posture. **Codex P2 follow-up** in the same PR: added `if: ${{ !endsWith(github.actor, '[bot]') }}` to `commit-check.yml` so Dependabot's `dependabot/github_actions/...` branch names don't trip `cchk.toml`'s `allow_branch_types` enforcement; matches the existing skip-for-bot-actors pattern in `claude-code-review.yml:25`. Conventional Commits validation for the *message* is still enforced for bot PRs via Dependabot's own `commit-message.prefix: "ci"` + `include: "scope"` config, which produces `ci(deps): bump …` subjects out of the box. **Field-choice follow-up in PR #160:** the original `github.actor` filter doesn't fire when a human clicks "Update branch" on a Dependabot PR — `github.actor` becomes the human and the check runs anyway, then fails the branch-type regex. Switched to `github.event.pull_request.user.login`, which stays `dependabot[bot]` regardless of who triggered the run; surfaced when the first auto-Dependabot PR (#158) hit the failing path. **Step-split follow-up in the same PR (Codex P2):** the original job-level `if:` skipped *all* validation on bot PRs — so a human follow-up commit on a Dependabot branch would bypass message-format validation entirely (Dependabot's `commit-message.prefix` config only constrains its own commits, not later contributors). Refactored `commit-check.yml` into two steps: "Validate commit messages" runs always, "Validate branch name" carries the bot-skip filter. Two action invocations cost ~30s of extra CI; the trade is full validation coverage on every commit.
...
- **#77** in PR #148 — debouncer key migrated from `str` to a `DebounceKey = tuple[DebounceRole, str]` tuple where `DebounceRole = Literal["single", "moved-out", "moved-into"]`. Closes the moved-out vs single collision (move-out `A/.dropboxignore` -> `B/...` plus a created/modified event on `A/.dropboxignore` within the 100ms RULES debounce window previously collapsed under last-wins). The PR #120 `"moved-into:"` string prefix is gone — the tuple's role discriminator subsumes it. `Debouncer._pending` / `submit` / `_run` and `daemon._classify` updated; existing tests in `test_debounce.py` switched from string keys to tuple keys; new regression test `test_classify_moved_out_and_single_same_path_have_distinct_roles` pins the contract.

- **#68** in PR #146 — cached the conflict count in `state.json` so `cli.status --summary` polling doesn't pay the per-tick rule-cache rglob. New `State.last_sweep_conflicts: int = 0` field with decode-tolerance for pre-#68 state.json (`.get(..., 0)`); `daemon._sweep_once` writes `len(cache.conflicts())` after the load_root pass (free — reads the already-computed `_conflicts` list from `_recompute_conflicts`); `cli.status` short-circuits in `--summary` mode and reads from state, only the human path runs the live walk to surface conflict details. Format string unchanged so the public-API contract holds. Three new tests including a `_load_cache must not be called` invariant pin.
...
- **#73** local-only (no PR — `.claude/settings.json` is gitignored at `.gitignore:3`) — fix candidate (B), the in-script guard. Two more fired triggers earlier in the session (PR #139 + PR #140 pre-flight `commit-check` loops) confirmed the `if` field's filtering wasn't behaving as expected; rather than diagnose further, the hook command now reads `tool_input.command` from stdin via `printf '%s' "$INPUT" | jq -r '.tool_input.command // empty'` and exits 0 unless the command begins with the literal `gh pr create`. The original `if` field is left in place as belt-and-suspenders. Verified with six test cases covering both over-fire shapes and actual-fire cases.

- **#34** in PR #139 — six-observation Windows CI flake retired via candidate (C). The daemon-driven multi-event smoke (`tests/test_daemon_smoke.py::test_daemon_reacts_to_dropboxignore_and_directory_creation`) was sensitive to silently-dropped `ReadDirectoryChangesW` events — diagnosed in PR #136 via the PR #135 instrumentation. New `tests/test_daemon_synthetic_events.py` reproduces the same coverage (rule-create + rule-modify + DIR_CREATE + conflict-detector WARNING via `caplog`) by firing stub events directly into `daemon._dispatch` with a real `RuleCache` against `FakeMarkers`; runs deterministically on every platform. The smoke test stays as a single-event Windows-only canary with a 10s budget and a failure message that names the diagnosed cause; if that ever flakes again, the kill-the-test path is preferred over further widening. The `_stub_event` helper from `tests/test_daemon_dispatch.py` was promoted to `tests/conftest.py:stub_event`.

- **#14 + #78 + #79** in PR #136 — daemon singleton hardening + flake retirement, bundled. **#78:** `daemon.run` now acquires an OS lock at `user_state_dir() / "daemon.lock"` (`fcntl.flock` on POSIX, `msvcrt.locking` on Windows) before any state-related work. Held for the daemon's lifetime, kernel-released on process exit. The prior state-based singleton check (read state.json → `_is_other_live_daemon`) is removed in favor of the atomic lock. **#79:** `State` gains `daemon_create_time: float | None`; `is_daemon_alive(pid, create_time=None)` extends to compare `psutil.Process(pid).create_time()` to the stored value within 1ms when supplied; falls back to the legacy substring-name check otherwise. `daemon.run` captures its own create_time at startup and threads it through `_sweep_once` to `state.write` so the persisted state.json carries the (pid, create_time) pair. **#14** closes as a side effect: the prior subprocess-spawn-based `test_run_refuses_when_another_pid_is_alive` (the flake) is replaced with `test_run_refuses_when_singleton_lock_is_held` plus three lock-helper unit tests — deterministic, no subprocess, no platform-conditional timing. Five new test cases total; existing test stubs (`lambda pid: True/False`) widened to accept the new kwarg. `cli._purge_local_state` cleans `daemon.lock` alongside `state.json` and `daemon.log` on `uninstall --purge`.

- **#44 + #82** in PR #134 — install-layer correctness fixes for unusual install paths, bundled. **#44:** `build_task_xml` now passes `getpass.getuser()`, `str(exe_path)`, and `arguments` through `xml.sax.saxutils.escape` so paths/usernames containing `&`, `<`, or `>` produce well-formed XML that `schtasks /Create /XML` accepts. **#82:** `build_unit_content` quotes the `ExecStart` path in double quotes (with C-style `\"` / `\\` escapes for embedded quotes/backslashes) when the path contains whitespace or other escape-needing chars; bare paths stay unquoted to match the on-disk shape stock distro installs have today. Literal `%` in the path is doubled to `%%` regardless of quoting so systemd's `%X` specifier expander does not silently rewrite the install path (added in response to a Codex bot review on PR #134). A literal `$` mid-path is passed through (NOT doubled) because systemd's bare `$VAR` form is only expanded when it is the entire argument; pass-through tests pin this contract. Helper `_quote_exec_start_path` is the seam; `_escape_systemd_env_value` was renamed to `_escape_systemd_quoted_string` since the same C-style escape rules apply to both `Environment="KEY=VALUE"` and `ExecStart="/path with space"`. Eleven new tests across `tests/test_install.py` (XML escape) and `tests/test_linux_systemd.py` (quoting + percent-escape + dollar-passthrough branches); the dollar tests were initially `$$`-escape assertions before a follow-up bot review corrected the systemd parser semantics, so the final shape is pass-through.

- **#66 + #67** in PR #133 — two cli.py log-hygiene/display fixes bundled. **#66:** dropped the `if discovered and ...` short-circuit in `cli.generate` so the "not under any discovered Dropbox root" warning fires when `_discover_roots()` returns `[]`. `find_containing(any, [])` already returns `None`, so the existing inner check covers the no-roots case with no message change. **#67:** `_apply_from_gitignore` now passes `log_warnings=False` to `cache.load_external`, restoring stderr parity with the regular `apply` path (PR #92's `_load_cache` precedent). Regression tests in `tests/test_cli_generate.py` and `tests/test_cli_apply.py` lock both contracts; both new tests verified to fail on un-fixed code before applying the fixes.
