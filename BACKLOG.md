...
**Status: RESOLVED 2026-05-14 (PR #<THIS_PR>).** Took fix candidate 1. `set_ignored` now tracks successfully-written attr names in a `written` list; on any `setxattr` failure it clears the already-written attrs (per-attr `removexattr`, `symlink=True`) before re-raising the original exception, so a dual-attr path is left either fully marked or not marked at all — never half-marked. A failure during the rollback clear is logged at WARNING and does not mask the original write exception. The docstring's old "partial state propagates" paragraph is rewritten to describe the rollback and to name *why* half-marked state is harmful (`is_ignored`'s first-hit-wins short-circuit hides it from the next sweep's retry). Two new tests in `tests/test_macos_xattr_unit.py`: `test_set_ignored_rolls_back_first_attr_on_second_write_failure` (both-mode, second `setxattr` raises `EDQUOT` after the first succeeds — asserts the first attr is cleared and `EDQUOT` propagates) and `test_set_ignored_rollback_failure_does_not_mask_original_error` (rollback `removexattr` itself raises `EACCES` — asserts the original `EDQUOT` still propagates). No user-visible CLI surface change, so no manual-test-script extension needed.
...
- **#125** (2026-05-14, PR #<THIS_PR>) — `set_ignored` (`_backends/macos_xattr.py`) now restores the atomic "both attrs set or neither" invariant its docstring claimed. Pre-fix, in dual-attr mode a first-write success followed by a second-write failure left the first xattr set; `is_ignored`'s first-hit-wins short-circuit then reported the path as fully ignored, so the next sweep never retried the missing second write — leaving the path marked for one sync stack but not the other. Fix candidate 1: track written attr names, and on any `setxattr` failure clear the already-written ones (`removexattr`, `symlink=True`) before re-raising the original exception. A rollback-clear failure is logged at WARNING without masking the original write exception. Single-attr mode is unaffected (nothing written yet when the lone write fails, so the rollback loop is a no-op). Two new tests in `tests/test_macos_xattr_unit.py` pin the rollback and the rollback-failure-doesn't-mask-original behavior. No user-visible CLI surface change. Surfaced 2026-05-13 in the external-review pass that produced #122 / #123 / #124 / #126.
