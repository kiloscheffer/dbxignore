...
- **#85** — `codex-followup.yml` cannot push workflow-file changes because the default GITHUB_TOKEN is always refused by GitHub for `.github/workflows/**` pushes regardless of `permissions:` (`workflows` is not a valid GITHUB_TOKEN scope key). When Codex flags a workflow-file finding, the bot stages a correct fix but the push fails. Fix-attempt cost (~$0.45/run) is wasted; the maintainer ends up pushing the equivalent fix manually. Lowest-blast-radius fix is a ~10-line prompt addition routing workflow-file findings to push-back-with-reasoning instead of fix-and-push. Surfaced 2026-05-08 in PR #160.
