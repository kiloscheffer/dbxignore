...
- **Conflict detector no longer drops effective negations under children-only patterns.** Previously, `build/*` followed by `!build/keep/` (the git-canonical pattern for "exclude all of `build/` except `build/keep`") was reported as a conflict and the negation was dropped — dbxignore marked `build/keep` ignored, diverging from git's behavior on the same rules. The detector now distinguishes directory negations whose target is matched by an earlier include (pathspec last-match-wins handles these correctly) from negations whose path lives strictly under a marked ancestor directory (Dropbox's inheritance makes those inert). Implemented via a `strict` flag on `_ancestors_of` (skip the target itself for directory negations whose `raw == prefix`) and last-match-wins logic in `_find_masking_include` (account for negations between an earlier include and the current rule). Behaviour change: `build/` + `!build/keep/` continues to flag (true conflict — `build/` marks the dir itself, inheritance wins); `build/*` + `!build/keep/` and `build/*` + `!build/keep/` + `!build/keep/**` no longer flag and now take effect, matching git.
