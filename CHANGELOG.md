...
- **Mixed-case `.dropboxignore` filenames are recognized end-to-end.** A new `is_ignore_filename(name)` predicate (case-insensitive `.dropboxignore` check) and `_canonical_cache_key(path)` helper (lowercase-basename normalization) replace exact-case `name == IGNORE_FILENAME` checks at every site that recognizes rule files: `match`, `explain`, `reload_file`, `remove_file`, `_load_file`, `_load_if_changed`, `daemon._classify`, `daemon._moved_dest_under_root`, `daemon._dispatch`'s move-event arm, `reconcile._reconcile_path`'s rule-file-marked-ignored warning, and `cli._walk_marked_paths`'s skip-rule-files filter. End state: a `.DropboxIgnore` (mixed casing on disk) is treated identically to `.dropboxignore` across discovery, watchdog events, cache mutations, and reconcile walks regardless of filesystem case sensitivity. Pre-fix, watchdog edits to mixed-case files had up-to-1-hour staleness on case-sensitive filesystems (Linux ext4, case-sensitive APFS) because `_classify` filtered them out; the cache also held entries under non-canonical keys after `reload_file`, so subsequent `match()` lookups silently missed. Resolves BACKLOG #92.
