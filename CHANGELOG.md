...
Alpha pre-release for beta-tester validation on real macOS hardware. Supersedes `0.4.0a3` (which made the macOS binary launch but didn't actually make the daemon do useful work) with two further fixes:

- **launchd plist invocation fix** (PR #76). v0.4.0a3 wrote a launchd plist that ran `/usr/local/bin/dbxignore` (the long-form CLI) with no subcommand on every spawn — Click printed help and exited with status 2, KeepAlive retried the same loop forever. Fixed by routing the frozen `detect_invocation` through the `dbxignored` daemon-shim sibling that ships next to the long-form binary. Linux installs were always correct (they use the non-frozen branch which already routed via `shutil.which("dbxignored")`); the bug was a frozen-branch asymmetry on macOS + Windows PyInstaller paths.
- **File Provider mode auto-detection** (PR #77). Modern Dropbox on macOS (default since 2023) syncs via Apple's File Provider extension and recognizes a different ignore attribute (`com.apple.fileprovider.ignore#P`) than legacy mode (`com.dropbox.ignored`). v0.4.0a3 hardcoded the legacy attribute, silently no-op'ing on every File Provider install. The xattr backend now queries Apple's PluginKit registry via `pluginkit -m -A -i com.getdropbox.dropbox.fileprovider` to detect which mode is active, falling back to a path-existence heuristic if pluginkit isn't reachable. Cached after first call so the per-file reconcile loop pays the ~50ms detection cost exactly once per process.

Also supersedes `0.4.0a2` (macOS bundling regression — `ModuleNotFoundError: No module named '_cffi_backend'`, fixed in PR #71) and the earlier-yanked `0.4.0a1` (the PyPI-publish-on-alpha misfire fixed in PR #62). Promotes to `[0.4.0]` once the beta tester signs off on the 10-step checklist from the v0.4 spec § "Beta-test workflow".

Brief summary of what's landed across PRs #53 – #77:
...
- **macOS xattr backend auto-detects Dropbox sync mode** (PR #77). Modern Dropbox on macOS runs in two distinct modes: legacy mode (Dropbox folder at `~/Dropbox`, synced by Dropbox's own daemon, watches `com.dropbox.ignored`) and File Provider mode (Dropbox folder at `~/Library/CloudStorage/Dropbox/`, synced by Apple's File Provider extension via `DropboxFileProvider.appex`, watches `com.apple.fileprovider.ignore#P`). File Provider has been the default since 2023; v0.4.0a3 hardcoded the legacy attribute name, silently no-op'ing on every File Provider install (markers got written but Dropbox didn't watch for them). Detection now queries Apple's PluginKit registry via `pluginkit -m -A -i com.getdropbox.dropbox.fileprovider` (path-independent, ~50ms one-time cost, robust to relocated File Provider folders) and falls back to a path-existence heuristic on `~/Library/CloudStorage/Dropbox/` when pluginkit is absent (e.g., non-macOS test hosts). Whitespace/`+` prefix in pluginkit output = active extension, `-` prefix = user explicitly disabled. Cached after first call. Output format and timing verified against macOS Tahoe 26.4 / Dropbox 250.4 on the beta tester's machine.
