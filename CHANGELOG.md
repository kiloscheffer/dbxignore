...
- Windows Explorer right-click integration. `dbxignore install` now registers
  ...
  `--yes` (confirmation via a native Windows MessageBox dialog), "Restore"
  runs with `--yes` (one-click safe). On Linux and macOS the flag is silently
  accepted as a no-op so portable scripts work unchanged. (#65)

### Changed

- **Breaking** — `dbxignored` removed as a separate entry-point. The daemon is now invoked via `dbxignore daemon` (a subcommand of the main CLI). Before: `pip install dbxignore` produced both `dbxignore` and `dbxignored` console scripts, and the GitHub Release shipped both `dbxignore.exe` and `dbxignored.exe` PyInstaller binaries. After: one `dbxignore` console script and one `dbxignore` Mach-O on macOS. Windows ships two binaries — `dbxignore.exe` (console-subsystem; the CLI you run interactively) and `dbxignorew.exe` (GUI-subsystem; Task Scheduler daemon entry + Explorer right-click verb invocations) — following the well-known `python.exe` / `pythonw.exe` pattern. All user-facing commands remain `dbxignore`; `dbxignorew.exe` is invoked by the system, not by you. The shell-integration verbs ("Ignore from Dropbox" / "Restore to Dropbox" per BACKLOG #65) route through `dbxignorew.exe` and use native Windows MessageBox dialogs for confirmation and error reporting. Resolves BACKLOG #30. (#30)
