...
- A uv-built virtualenv's `pythonw.exe` is a byte-identical copy of the console-subsystem `python.exe` trampoline — the name does *not* mean GUI-subsystem. Anything that needs a windowless Windows launcher must verify the PE Subsystem field (`_common._is_gui_subsystem`) or use the `dbxignorew` `[project.gui-scripts]` trampoline, never trust the `pythonw.exe` filename.
