...
Operational notes too situational for `CONTRIBUTING.md`'s top-level Gotchas list.
Read the matching section before touching that area.
...
- Windows windowless-launcher selection: never trust a filename, always verify the PE Subsystem field (`_common._is_gui_subsystem`). uv *project* venvs (`uv sync` / `uv run`) ship 47 KB **console-subsystem** trampolines for *both* `python.exe` and `pythonw.exe` — the `pythonw.exe` name does not mean GUI-subsystem there. `uv tool install` venvs (and stdlib `python -m venv`) ship a genuine GUI-subsystem `pythonw.exe`. The `dbxignorew` `[project.gui-scripts]` trampoline is *itself* GUI-subsystem but re-execs the sibling `pythonw.exe`, so it is windowless only when that `pythonw.exe` is — `_common._dbxignorew_launches_windowless` is the check. Net effect: `uv tool install` / `pip install` / frozen `.exe` give a windowless daemon; `uv run dbxignore install` from a source checkout cannot (uv project venvs have no windowless launcher) and `detect_invocation` deliberately falls through to a WARNING there.
...
- Swapping a high-level write primitive (`Path.write_text`, text-mode `open(...)`) for a lower-level shape (`tempfile.mkstemp` + `os.fdopen` + `os.replace`) silently loses inherited defaults. Two concrete examples: text-mode `\n`→`\r\n` translation on Windows (pin with `newline=""`) and umask-based file mode on POSIX (mkstemp creates at `0o600`; restore via `os.chmod` to the existing file's mode or `0o666 & ~umask` for a new file). Audit each invariant the high-level primitive carries via its defaults before reaching for the low-level shape; preserve or change explicitly, never inherit silently.
